...
*/
function fn_get_categories_resp_mobile()
{
    $fields = "cd.category_id, cd.category, concat(csn.name, '.html') as url";
    $categories = db_get_array("SELECT ". $fields." FROM cscart_categories cc
                        LEFT JOIN cscart_category_descriptions cd ON cc.category_id = cd.category_id
                        inner join cscart_seo_names csn on csn.object_id=cc.category_id
                        WHERE cc.status='A' and csn.type='C'
                        AND cc.parent_id=0 order by position asc");
    return $categories;
}
...
*/
function fn_get_subcategories_resp_mobile($cat_id)
{
    $subcategories = db_get_array("SELECT cd.category_id, cd.category
                                    FROM cscart_categories cc
                                    LEFT JOIN cscart_category_descriptions cd ON cc.category_id = cd.category_id
                                    WHERE cc.status='A'
                                    AND cc.parent_id=".$cat_id." order by position asc");
    return $subcategories;
}


/*
 * Get list price by product id
*/
function fn_get_product_list_price($product_id)
{
    $price = db_get_field("SELECT list_price FROM ?:products WHERE product_id = ?i", $product_id);
    return $price;
}

/*
 * Get deal price by product id
*/
function fn_get_product_deal_price($product_id)
{
    $price = db_get_field("SELECT third_price FROM ?:products WHERE product_id = ?i", $product_id);
    return $price;
}
...
*/
function fn_get_product_selling_price($product_id)
{
    $price = db_get_field("SELECT price FROM ?:product_prices WHERE product_id = ?i", $product_id);
    return $price;
}

function is_review_enabled(){
    if(Registry::get('config.new_review_enable')){
        $new_review_enabled = true;
    }
    else{
        $new_review_enabled = false;
    }
    return $new_review_enabled;
}
...
//change by ankur to fix not showing product
       if(AREA=='A')
	   {
		   $field_list .= ',group_concat(?:categories.status) as cat_status,group_concat(?:categories.have_option) as have_option';
			$join .= " LEFT JOIN ?:products_categories ON ?:products_categories.product_id = ?:products.product_id LEFT JOIN ?:categories ON ?:categories.category_id = ?:products_categories.category_id $avail_cond";
	   ...
	   else
	   {
		...
		if (!empty($product_data['tracking']) && $product_data['tracking'] == 'O') {
			$product_data['amount'] = db_get_field("SELECT MAX(amount) FROM ?:product_options_inventory WHERE product_id = ?i", $product_id);
                ...
                //echo "<pre>"; print_r($product_data['amount']);die;
		...
		if ($features) {
			...
			$path = (!empty($product_data['main_category']))? explode('/', db_get_field("SELECT id_path FROM ?:categories IGNORE INDEX(p_category_id) WHERE category_id = ?i", $product_data['main_category'])) : "";
			...
			//change by ankur to not fetch the feature if main category is empty
			if(!empty($path))
			{
...
{       
   ...
   if ($params['get_options'] == true) {
	...
	foreach ($products as &$_product) { 
		...
		if ($params['get_options'] == true) {  
			...
			if (empty($product['product_options'])) {
				...
				$product['product_options'] = (!empty($selected_options)) ? fn_get_selected_product_options($product['product_id'], $selected_options, CART_LANGUAGE) : $product_options[$product_id];
                       ...
                       //echo "<pre>"; print_r($product['product_options']); 
		...
		/* 
		****Removed catalog promotion on products and category page by chandan to completly kill the catalog promotion.****
...
function fn_get_product_files($product_id, $preview_check = false, $order_id = 0, $lang_code = DESCR_SL)
{
	...
	fn_set_hook('get_product_files', $product_id, $order_id, $fields, $join, $condition);
			...
			// Changed by Sudhir dt 31st octo 2012 to make query memcached start here
			if(Registry::get('config.memcache') && $GLOBALS['memcache_status'])
			{
				$memcache = $GLOBALS['memcache'];
				$key = md5("SELECT " . implode(', ', $fields) . " FROM ?:product_files ?p ?p ORDER BY position. $join. $condition");
				if($mem_value = $memcache->get($key)){
					...
					$status = $memcache->set($key, $files, MEMCACHE_COMPRESSED, Registry::get('config.memcache_expire_time'));
		                        if(!$status){
		                            $memcache->delete($key);
		                        }
				}
			...
			// Changed by Sudhir dt 31st octo 2012 to make query memcached
	if (!empty($files)) {
...
function fn_get_categories($params = array(), $lang_code = CART_LANGUAGE)
{
	...
	$fields = array (
		...
		'?:categories.position',
		'?:categories.status',
		'?:categories.show_as_new', //modified by clues dev
		'?:categories.is_deal_category',
                '?:categories.banner_html',
        '?:categories.show_to_merchant'
	);

	if ($params['simple'] == false) {
		//change by ankur to get product count of category
		if(AREA=='A')
		{
			$fields[] = 'count(distinct(pc.product_id)) as product_count'; 
		}
		else
		{
	...
	if ($params['visible'] == true && empty($params['b_id'])) {
		if (!empty($params['current_category_id'])) {
			$cur_id_path = db_get_field("SELECT id_path FROM ?:categories IGNORE INDEX(p_category_id) WHERE category_id = ?i", $params['current_category_id']);
	...
	if(isset($params['escape_cat_id']) && is_array($params['escape_cat_id'])  && count($params['escape_cat_id'])>0){
		$escape_cat_id = "'".implode("','",$params['escape_cat_id'])."'";
		//$condition .= db_quote(" AND ?:categories.category_id not in (".$escape_cat_id.") AND ?:categories.parent_id  not in (".$escape_cat_id.")");	
		$condition .= db_quote(" AND (?:categories.show_to_merchant >= 'Y' )"); //modified by chandan to remove category access from merchant
	...
	//change by ankur to get only active category if param is true
	if(isset($params['show_only_active']) && $params['show_only_active'])
	{
		$condition.=db_quote(" AND ?:categories.status='A'");
	}
    //code end
	
	//added by ankur to get the category product count
	if(AREA=='A')
	{
		$join.="left join cscart_products_categories pc on pc.category_id=?:categories.category_id";
	...
	/*if(Registry::get('config.memcache') && $GLOBALS['memcache_status'])
    	{
        	$memcache = $GLOBALS['memcache'];
        	$key = md5('SELECT ' . implode(',', $fields) . " FROM ?:categories LEFT JOIN ?:category_descriptions ON ?:categories.category_id = ?:category_descriptions.category_id AND ?:category_descriptions.lang_code = ?s $join WHERE 1 ?p $group_by ORDER BY $sorting ?p".'category_id'.$lang_code.$condition.$limit);
        	if($mem_value = $memcache->get($key)){
            		$categories = $mem_value;        
	        }else{
	            $categories = db_get_hash_array('SELECT ' . implode(',', $fields) . " FROM ?:categories LEFT JOIN ?:category_descriptions ON ?:categories.category_id = ?:category_descriptions.category_id AND ?:category_descriptions.lang_code = ?s $join WHERE 1 ?p $group_by ORDER BY $sorting ?p", 'category_id', $lang_code, $condition, $limit);
        	    $status = $memcache->set($key, $categories, MEMCACHE_COMPRESSED, Registry::get('config.memcache_expire_time')); // or die ("Failed to save data at the server");
                    if(!$status){
                        $memcache->delete($key);
                    }
	...
	if ($params['simple'] == true || $params['group_by_level'] == true) {
		...
		{
			$memcache = $GLOBALS['memcache'];
			$key = md5("SELECT category_id, parent_id FROM ?:categories WHERE parent_id IN(?n) ?p".'parent_id'.$child_for.$where_condition);
			if($mem_value = $memcache->get($key)){
				$has_children = $mem_value;
			}else{
				$has_children = db_get_hash_array("SELECT category_id, parent_id FROM ?:categories WHERE parent_id IN(?n) ?p", 'parent_id', $child_for, $where_condition);
				$status = $memcache->set($key, $has_children, MEMCACHE_COMPRESSED, Registry::get('config.memcache_expire_time')); // or die ("Failed to save data at the server");
                                if(!$status){
                                    $memcache->delete($key);
                                }
			}
...
function fn_get_category_data($category_id = 0, $lang_code = CART_LANGUAGE, $field_list = '', $get_main_pair = true)
{
    ...
    {
        $memcache = $GLOBALS['memcache'];
        $key = md5($category_id.'-category_data');
        if($mem_value = $memcache->get($key)){
            ...
            $memcache->set($key, $category_data, MEMCACHE_COMPRESSED, Registry::get('config.memcache_long_expire_time')); // or die ("Failed to save data at the server");
        }
    }else{
        if (defined('COMPANY_ID')) {
		$company_data = Registry::get('s_companies.' . COMPANY_ID);
...
function fn_delete_product($product_id)
{
	...
	if (!empty($product_id)) {
		...
		// fn_rm(DIR_DOWNLOADS . $product_id); //Commented by Sudhir dt 08 july 2013 to improve server performance
...
function fn_update_category($category_data, $category_id = 0, $lang_code = CART_LANGUAGE)
{
	...
	if (empty($category_id)) {
		...
		if ($parent_id == 0) {
			...
			$id_path = db_get_row("SELECT id_path FROM ?:categories IGNORE INDEX(p_category_id) WHERE category_id = ?i", $parent_id);
...
function fn_change_category_parent($category_id, $new_parent_id)
{
	if (!empty($category_id)) {
		...
		$new_parent_path = db_get_field("SELECT id_path FROM ?:categories IGNORE INDEX(p_category_id) WHERE category_id = ?i", $new_parent_id);
		$current_path = db_get_field("SELECT id_path FROM ?:categories IGNORE INDEX(p_category_id) WHERE category_id = ?i", $category_id);
...
function fn_get_product_options($product_ids, $lang_code = CART_LANGUAGE, $only_selectable = false, $inventory = false, $only_avail = false)
{
	...
	if (!empty($product_ids)) { 
               //commented by ankur because of pbm in product option in front end
             /*  if(Registry::get('config.memcache') && $GLOBALS['memcache_status']) 
                {
                  $memcache = $GLOBALS['memcache']; 
                  $value = "SELECT a.*, b.option_name, b.option_text, b.description, b.inner_hint, b.incorrect_message, b.comment,b.staff_notes FROM ?:product_options as a LEFT JOIN ?:product_options_descriptions as b ON a.option_id = b.option_id AND b.lang_code = ?s WHERE a.product_id IN (?n)".$condition.$_status."ORDER BY a.position".array('product_id', 'option_id').$lang_code . $product_ids.$_SERVER['REQUEST_URI'];
                  $key = md5($value);
                  if($mem_value = $memcache->get($key)){
                      $_options = $mem_value;
                  }else{ 
                      ...
                      $status = $memcache->set($key,$_options,MEMCACHE_COMPRESSED,Registry::get('config.memcache_expire_time'));
                      if(!$status){
                          $memcache->delete($key);
                       }
                     }
...
//Added by Shashi kant to remove variants with zero inventory
  if(Registry::get('config.variants_with_zero_inventory')==1){      
  $inventory_data = db_get_array("Select * from cscart_product_options_inventory where product_id = '".$product_ids[0]."' and amount>0");
   
    foreach($inventory_data as $key=> $row){
                 $comb = explode('_', $row['combination']);
                 $size = count($comb);
                     for($i=1;$i< $size;$i+=2){
                         $final_arr[$comb[$i]] = $row; 
                     }
                   }

             $_amount=array();
             $zero_amount=array();
             foreach ($final_arr as $key => $var_id) {
                 array_push($_amount,$key);
                  }

               foreach($variants as $key1=> $value1){    
                      foreach ($value1 as $key2=> $val) {
                          array_push($zero_amount, $key2);
                        }
                      } 
                    
             $zero_amount = array_values(array_unique($zero_amount));
             $_amount = array_values(array_unique($_amount));
           
             $array_unset = array_diff($zero_amount, $_amount);
             
                  foreach($array_unset as $key3 => $var){
                      foreach($variants as $key4=> $value2){
                          foreach ($value2 as $key5=> $val1) {
                               if($var == $key5) {
                                   unset($variants[$key4][$key5]); 
                                  }
                               }
                            }
                         }
  }  
//End Added by Shashi kant to remove variants with zero inventory
...
{       
	...
	if(!Registry::get('config.global_otions_query_optimize')){
	$_opts = db_get_array("SELECT a.option_id, a.option_type, a.position, a.inventory, a.product_id, a.regexp, a.required, a.multiupload, a.allowed_extensions, a.max_file_size, b.option_name, b.option_text, b.description, b.inner_hint, b.incorrect_message, b.comment, a.status FROM ?:product_options as a LEFT JOIN ?:product_options_descriptions as b ON a.option_id = b.option_id AND b.lang_code = ?s LEFT JOIN ?:product_global_option_links as c ON c.option_id = a.option_id WHERE (a.product_id = ?i OR c.product_id = ?i) AND a.status = 'A' ORDER BY a.position", $lang_code, $product_id, $product_id);
            ...
            $_opts = db_get_array("SELECT a.option_id, a.option_type, a.position, a.inventory, a.product_id, a.regexp, a.required, a.multiupload, a.allowed_extensions, a.max_file_size, b.option_name, b.option_text, b.description, b.inner_hint, b.incorrect_message, b.comment, a.status FROM ?:product_options as a LEFT JOIN ?:product_options_descriptions as b ON a.option_id = b.option_id AND b.lang_code = ?s LEFT JOIN ?:product_global_option_links as c ON c.option_id = a.option_id WHERE  c.product_id = ?i AND a.status = 'A' ORDER BY a.position", $lang_code, $product_id, $product_id);
            $_opts1 = $_opts = db_get_array("SELECT a.option_id, a.option_type, a.position, a.inventory, a.product_id, a.regexp, a.required, a.multiupload, a.allowed_extensions, a.max_file_size, b.option_name, b.option_text, b.description, b.inner_hint, b.incorrect_message, b.comment, a.status FROM ?:product_options as a LEFT JOIN ?:product_options_descriptions as b ON a.option_id = b.option_id AND b.lang_code = ?s  WHERE a.product_id = ?i  AND a.status = 'A' ORDER BY a.position", $lang_code, $product_id, $product_id);

                $_opts = array_merge($_opts, $_opts1);
/*
    if(empty($_opts)) {
        $_opts = db_get_array("SELECT a.option_id, a.option_type, a.position, a.inventory, a.product_id, a.regexp, a.required, a.multiupload, a.allowed_extensions, a.max_file_size, b.option_name, b.option_text, b.description, b.inner_hint, b.incorrect_message, b.comment, a.status FROM ?:product_options as a LEFT JOIN ?:product_options_descriptions as b ON a.option_id = b.option_id AND b.lang_code = ?s  WHERE a.product_id = ?i  AND a.status = 'A' ORDER BY a.position", $lang_code, $product_id, $product_id);
...
{        
	...
	$base_fields = $fields = array (
		...
		'?:product_features.is_key',
		'?:product_features.is_filter_feature',  // added by ankur to get whether feature is filter feature or not
		'?:product_features.required_feature'  // added by ankur to get whether feature is required feature or not
	);
	...
	if (!empty($params['category_ids'])) {
		...
		foreach ($c_ids as $k => $v) {
			//$find_set[] = db_quote(" FIND_IN_SET(?i, ?:product_features.categories_path) ", $v);
                        $find_set[] = db_quote(" ?:product_features.categories_path LIKE  '%".$v."%'");
	...
	if (!empty($params['only_is_key'])) {
		$condition .= db_quote(" AND ?:product_features.is_key = '1' ");
	}
	//code by ankur for the new search element
	if (!empty($params['search_for'])) {
		if($params['search_for']=='G')
		{
			$condition .=db_quote(" AND ?:product_features.feature_type='G'");
		}
		else if($params['search_for']=='GF')
		{
			if(!empty($params['GF_type'][0]) && !empty($params['GF_type'][1]))
			{
				$condition .=db_quote(" AND ?:product_features.is_filter_feature!='G'");
			}
			else if($params['GF_type'][0]=='gf')
			{
				$condition .=db_quote(" AND ?:product_features.is_filter_feature!='Y'");
			}
			else if($params['GF_type'][0]=='ff')
			{
				$condition .=db_quote(" AND ?:product_features.is_filter_feature='Y'");
			}
		}
	...
	//code by ankur to sort feature 
	   $sort_by="group_position,?:product_features.position, ?:product_features_descriptions.description"; //default setting
	   $sort_order="";
	 if(isset($params['sort_by']) && !empty($params['sort_by']))
	 {
		 if($params['sort_by']=='feature')
		 {
			 $sort_by="?:product_features_descriptions.description";
		 }
		 else if($params['sort_by']=='status')
		 {
			 $sort_by="?:product_features.status";
		 }
	 }
	...
	$data = db_get_hash_array("SELECT " . implode(', ', $fields) . " FROM ?:product_features $join WHERE 1 $condition $group ORDER BY $sort_by $sort_order $limit", 'feature_id');
	
	//changes by ankur to fix search on feature page
	if(CONTROLLER=='product_features')
	$has_ungroupped = true;
	else
	...
	if (!empty($data)) {
		if ($params['variants'] == true) {
			if(AREA == 'C'){
				...
				foreach ($data as $k => $v)
				{
					...
					$feature_id_str = $feature_id_str . $v['feature_id'];
				}
				$arr_res = fn_get_product_feature_variants_customerside($feature_id_str, $params['product_id'], $v['feature_type'], true, $lang_code);
				foreach ($data as $k => $v) {
					if (in_array($v['feature_type'], array('S', 'M', 'N', 'E'))) {
						$arr_cur = array();
						foreach($arr_res as $key_res => $val_res)
						{
							if($val_res['feature_id']==$v['feature_id'])
							{
								$arr_cur[] = array($key_res=>$val_res);
							}
						}
						if(count($arr_cur)==1)
						{
							$data[$k]['variants'] = $arr_cur[0];
						}
						else
						{
							foreach($arr_cur as $arr)
							{
								foreach($arr as $key_2=> $val_2)
								{
									$data[$k]['variants'][$key_2] = $val_2;
								}
								
							}
						}
						//$data[$k]['variants'] = fn_get_product_feature_variants_customerside($v['feature_id'], $params['product_id'], $v['feature_type'], true, $lang_code);
		...
		if ($params['plain'] == false) {
			...
			foreach ($data as $k => $v) {
				...
				//code by ankur to show display on if feature doesn't have any category
				else if(!empty($params['get_descriptions']) && !empty($v['parent_id']))
				{
					$d = fn_get_categories_list($v['categories_path'],$v['parent_id']);
					$data[$k]['feature_description'] = fn_get_lang_var('display_on') . ': <span>' . implode(', ', $d) . '</span>';
				}
				//code end
			}
...
function fn_get_product_feature_variants($feature_id, $product_id, $feature_type, $get_images = false, $lang_code = CART_LANGUAGE)
{
	...
	// Changed by Sudhir dt 29th octo 2012 to optimize query bigin here
	...
	$vars = db_get_hash_array('SELECT ' . implode(', ', $fields) . " FROM ?:product_feature_variants $join WHERE 1 $condition ORDER BY $sorting", 'variant_id');
...
function fn_get_product_feature_variants_customerside($feature_id, $product_id, $feature_type, $get_images = false, $lang_code = CART_LANGUAGE)
{
	
	$sql = "select d.*,v2.*,v.variant_id as selected,cscart_product_features.feature_type
			from cscart_product_features_values v
			left join cscart_product_feature_variant_descriptions d on v.variant_id=d.`variant_id`
			left join cscart_product_feature_variants v2 on v2.`variant_id`=v.variant_id
			LEFT JOIN cscart_product_features ON cscart_product_features.feature_id = v2.feature_id
			where v.product_id=".$product_id." and v.feature_id in (".$feature_id . ") and v.lang_code ='".$lang_code."'";
	$vars = db_get_hash_array($sql,'variant_id');
...
function fn_get_product_filters($params = array(), $items_per_page = 0, $lang_code = DESCR_SL)
{
	...
	$find_set[] = db_quote(" ?:product_filters.categories_path LIKE '%".$v."%'");
		}
		$find_in_set = db_quote(" AND (?p)", implode('OR', $find_set));
		$condition .= $find_in_set;	
	}
...
function fn_get_filters_products_count($params = array(), $block_id=false)
{
    if(isset($params['category_id']) && $params['category_id'] != ''){
            $is_nrh = db_get_field("select is_nrh from cscart_categories where category_id =".$params['category_id']);
            if($is_nrh == 'Y' || $params['category_id'] == Registry::get('config.nrh_root_category_id')){
                    return array();
            }
    }
  if(Registry::get('config.solr') != 'true' || $params['dispatch'] == 'index.index' || $params['dispatch'] == 'categories.view') {
	...
	//if (Registry::is_exist($key) == false) {
		if (!empty($params['check_location'])) { // FIXME: this is bad style, should be refactored
			...
			if(($params['dispatch'] == 'index.index') && is_array($_REQUEST['features_hash']) && !empty($_REQUEST['company_id']) && empty($_REQUEST['category_id'])){
				$params['dispatch'] = $_REQUEST['dispatch'];
			}
			...
			elseif ($params['dispatch'] == 'products.search') {				
				$params = $_REQUEST;
				...
				list($products, $search, $products_count) = fn_get_products($params);

				$category_name = fn_get_category_name($search['cid']);
				$content = $params['q'].':'.count($products).':'.(($category_name)? $category_name : 'All').':'. date('d-M-Y h-i-s');
				log_to_file('search_keyword',$content);
				//echo '<pre>product count : '.count($products);print_r($params);echo '</pre>';
				$params['category_id']= array();
				foreach($products as $product){
					$categories = $product['category_ids'];					
					if(!in_array(key($categories), $params['category_id'])){
						$params['category_id'][] = key($categories);						
					}
		...
		if (!empty($params['category_id'])) {
			if ($params['dispatch'] != 'products.search') {
				$id_path = db_get_field("SELECT id_path FROM ?:categories IGNORE INDEX(p_category_id) WHERE category_id = ?i", $params['category_id']);
				...
				//$condition .= db_quote(" AND (categories_path = '' OR FIND_IN_SET(?i, categories_path))", $params['category_id']);				
                                $condition .= db_quote(" AND (categories_path = '' OR categories_path LIKE '%".$params['category_id']."%')");
                                $where .= db_quote(" AND ?:products_categories.category_id IN (?n)", $category_ids);
			} else {
				$category_ids = $params['category_id'];
				$product_ids = implode(',',$params['product_id']);
				$where .= db_quote(" AND ?:products.product_id IN ($product_ids)");
			}
		...
		//if (!empty($params['get_for_home']) //condition added by sudhir for category view
		if (!empty($params['get_for_home']) && (empty($params['category_id']) || !empty($params['company_id']))) {
			$condition .= db_quote(" AND ?:product_filters.show_on_home_page = ?s", $params['get_for_home']);
		}elseif($params['cid'] == 0 && $params['q'] !=''){
			$condition .= db_quote(" AND ?:product_filters.show_on_home_page = ?s", $params['get_for_home']);
		}
		...
		if (!empty($variants_ids)) {
			...
			if(is_array($params['features_hash'])){
		  	     foreach ($variants_ids as $k => $variant_id){
				$where_conditions = array();
				foreach ($variant_id as $x => $v_id) {
					//$where_conditions[] = db_quote(" FIND_IN_SET('?i', simple_variants)", $v_id);
					$where_conditions[] = db_quote(" simple_variants LIKE '%".$v_id."%'");
				...
				foreach ($variants_ids as $k => $variant_id) {
					//$where_condtions[] = db_quote(" FIND_IN_SET('?i', simple_variants)", $variant_id);
                                    	$where_condtions[] = db_quote(" simple_variants LIKE '%".$variant_id."%'");
				}
		...
		// Added By Sudhir dt 25th Sept 2012 for merchant microsite shopping options 
		if($_REQUEST['company_id'] AND $_REQUEST['category_id']){
			$where .= ' AND ?:products.company_id='.$_REQUEST['company_id'];
		}
		// Added By Sudhir dt 25th Sept 2012 for merchant microsite shopping options end here 
                /*added by chandan to add the support of market id in filters*/
                if(isset($_REQUEST['market_id']) && $_REQUEST['market_id'] != ''){
                $nrh_join = " Inner join clues_company_badge_answer as ccab on ccab.company_id=companies.company_id and ccab.badge_id='2' and ccab.question_id='".$_REQUEST['market_id']."'";
                }else{
                        $nrh_join = '';
                }
...
//echo '<pre>';print_r($field_filters);echo '</pre>';
...
{       
         /* Product filter delete log */
                  $product_data = 'filter_id'.''.$filter_id;
                  $action = 'Delete product filters';
                  $user_id = $_SESSION['auth']['user_id']; 
                  $action_date = date('Y-m-d h:i:s');
                  clues_product_filter_history($product_data, $action, $user_id, $action_date);
	   /* End */
...
function fn_parse_features_hash($features_hash = '', $values = true)
{
	if (empty($features_hash)) {
	    ...
	    if(is_array($features_hash)){
		$variants_ids = $ranges_ids = $fields_ids = array();
		   foreach($features_hash as $pr=>$sh_option){
			preg_match_all('/([A-Z]+)([\d]+-?\d*-?\w*)[,]?/', $sh_option, $vals);
			$op_val = explode(".",$sh_option);
			if ($values !== true) {
				return $vals;
			}
			if (!empty($vals) && !empty($vals[0]) && !empty($vals[1])) {
				foreach ($vals as $key => $range_type) {
					if ($range_type == 'V') {
						// Feature variants
			...
			if (!empty($op_val) && !empty($op_val[0]) && !empty($op_val[1])) {
				if ($op_val[0] != '1') {
					$variants_ids[$op_val[1]] = $op_val[0];
				}
			}
		   }
		...
		$variants_id = array();
		foreach($variants_ids as $va=>$vid){
			$variants_id[$vid][]=$va;
		}

		return array($variants_id, $ranges_ids, $fields_ids);
	    } else {
		$variants_ids = $ranges_ids = $fields_ids = array();
...
function fn_get_products($params, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
      //By Ajay items_per_page manage by config for mobile and web
       if(Registry::get('config.products_limit_per_page') && Registry::get('config.products_limit_per_page') != 0 ) {
            $items_per_page = Registry::get('config.products_limit_per_page');
        }
      // End by Ajay

	//HPRAHI Code start
	$arr_noise=Registry::get('config.noisewords');
	$arr_replacewithspace = Registry::get('config.replacewithspace');
	$str='';
	if(isset($params['q']))
	$str = $params['q'];
	foreach($arr_replacewithspace as $val)
	{
	...
	if (empty($params['only_short_fields'])) {
		// Edited by Sudhir dt 13th octo 2012 to optimize multi select search query
		if (($params['dispatch'] != 'products.search') AND ($params['dispatch'] != 'categories.view')) {
			$fields = array (
				'products.*',
                                'prices.retail_price'
			...
			$fields = array (
				'products.product_id,products.company_id, products.list_price, products.timestamp, products.free_shipping, products.amount, products.sdeep_rating_info, products.view_count, products.deals_index, products.feature_index,products.is_cod, products.last_update, products.transfer_price, products.special_offer_text, products.special_offer, products.one_day_sale_start_datetime, products.one_day_sale_end_datetime,products.one_day_sale_short_text,  products.promotion_id, products.price_see_inside, products.deal_inside_badge, products.special_offer_badge,products.freebee_inside,products.is_wholesale_product,prices.retail_price,products.why_buy_reason_1,products.why_buy_reason_2',
);
		}		// Edited by Sudhir dt 13th octo 2012 to optimize multi select search query end here
	} else {
		$fields = array (
			'products.product_id',
	...
	$sortings = array (
        ...
        'hotdeals' => 'products.third_price',// added by ajay//changed by sapna deals index to third price
		'newarrivals' => 'products.product_id',// added by ajay
		'featured' => 'products.feature_index',// added by ajay
		'company' => 'company_name',		
		'last_update' => 'products.last_update',
		'feature_index' => 'products.feature_index', // added by Sudhir dt 09/08/2012
		'deals_index' => 'products.deals_index', // added by Sudhir dt 11/08/2012
		'product_sales_amount' => 'cscart_product_sales.amount', // added by Sudhir 
		'clues_popularity' => 'pp.total', // added by Sudhir dt 16/01/2013
		'pro_popularity' =>'?:product_popularity.total',//added by ankur for the field of sorting at admin side
		'null' => 'NULL'
	);
	...
	if (isset($params['q']) && fn_string_no_empty($params['q'])) {
		...
		foreach ($pieces as $piece) {
			...
			if ($params['pkeywords'] == 'Y') {
				...
				$tmp .= db_quote(" OR (descr1.meta_keywords LIKE ?l )", "%$piece%");
			}
	...
	if (!empty($params['features_hash']) || (!fn_is_empty($params['variants']))) {
		// Edited by Sudhir dt 13th octo 2012 to optimize multi select search query
		if (($params['dispatch'] != 'products.search') AND ($params['dispatch'] != 'categories.view')) {
			$join.=db_quote(" LEFT JOIN ?:product_features_values ON ?:product_features_values.product_id=products.product_id ");
		} // Edited by Sudhir dt 13th octo 2012 to optimize multi select search query end here
	}
	...
	if (!empty($advanced_variant_ids)) {
		//$join .= db_quote(" LEFT JOIN (SELECT product_id, GROUP_CONCAT(?:product_features_values.variant_id) AS advanced_variants FROM ?:product_features_values GROUP BY product_id) AS pfv_advanced ON pfv_advanced.product_id = products.product_id");
		$join .= db_quote(" LEFT JOIN ?:product_features_values.variant_id AS pfv_advanced ON pfv_advanced.product_id = products.product_id");
		...
		foreach ($advanced_variant_ids as $k => $variant_ids) {
			...
			foreach ($variant_ids as $variant_id => $v) {
				...
				$where_or_conditions[] = db_quote(" pfv_advance.variant_id in ($variant_id)");
			}
	...
	if (!(empty($simple_variant_ids) && empty($ranges_ids))) {
		if($params['dispatch'] == 'product_features.view') {		
			$join .= db_quote(" LEFT JOIN ?:product_features_values AS simple_variants ON simple_variants.product_id = products.product_id");
		} else {
			foreach ($simple_variant_ids as $k_id => $v_id) {
				$var_id = implode(",",$v_id);
			}
			if($var_id){
				$variants = "variant_id in (".$var_id.")";
			}else{
				$variants = " 1 ";
			}
			$join .= db_quote(" LEFT JOIN (SELECT product_id, GROUP_CONCAT(?:product_features_values.variant_id) AS simple_variants FROM ?:product_features_values WHERE ".$variants." GROUP BY product_id) AS pfv_simple ON pfv_simple.product_id = products.product_id");
		}
	}
...
//		$join .= db_quote(" LEFT JOIN (SELECT product_id, GROUP_CONCAT(?:product_features_values.variant_id) AS simple_variants FROM ?:product_features_values WHERE lang_code = ?s GROUP BY product_id) AS pfv_simple ON pfv_simple.product_id = products.product_id", CART_LANGUAGE);
		...
		if (($params['dispatch'] != 'products.search') AND ($params['dispatch'] != 'categories.view')) {
			 $join .= db_quote(" LEFT JOIN ?:product_features_values AS pfv_simple1 ON pfv_simple1.product_id = products.product_id");			
		}
	   ...
	   if(is_array($params['features_hash'])){
		...
		foreach ($variant_id as $x => $v_id) {
			//$where_conditions[] = db_quote(" FIND_IN_SET('?i', simple_variants)", $v_id);
			//$where_conditions[] = db_quote(" simple_variants.variant_id in ($v_id)");
			$where_conditions[] = db_quote(" simple_variants like '%". $v_id."%'");
		}
		//$condition .= ' AND ('.implode(' OR ', $where_conditions).')';
		...
		foreach ($simple_variant_ids as $k => $variant_id) {
			...
			$where_conditions[] = db_quote(" simple_variants.variant_id in ($variant_id)");
			$where_conditions[] = $variant_id;
		}
		$condition .= ' AND '.implode(' AND ', $where_conditions);
		$condition .= '  AND pfv_simple1.variant_id in ('.implode(' ,', $where_conditions) .')';
	   }
	}
	...
	if (!empty($params['custom_range'])) {
		foreach ($params['custom_range'] as $k => $v) {
			...
			if (fn_string_no_empty($v['from']) || fn_string_no_empty($v['to'])) {
				...
				$join .= db_quote(" LEFT JOIN ?:product_features_values as custom_range_$k ON custom_range_$k.product_id = products.product_id ");
				if (fn_string_no_empty($v['from']) && fn_string_no_empty($v['to'])) {
	...
	if (!empty($ranges_ids)) {
		if(is_array($params['features_hash'])){
			$where_conditions = array();
			foreach ($ranges_ids as $k => $variant_id) {
				//$where_conditions[] = db_quote(" FIND_IN_SET('?i', simple_variants)", $variant_id);
				$where_conditions[] = db_quote(" simple_variants.variant_id in ($variant_id)");
			}
			$condition .= ' AND ('.implode(' OR ', $where_conditions).')';
			...
			foreach ($range_conditions as $k => $range_condition) {
				//$join .= db_quote(" LEFT JOIN ?:product_features_values as var_val_$k ON var_val_$k.product_id = products.product_id AND var_val_$k.lang_code = ?s", CART_LANGUAGE);
				$join .= db_quote(" LEFT JOIN ?:product_features_values as var_val_$k ON var_val_$k.product_id = products.product_id ");
	...
	if (!empty($params['ch_filters']) && !fn_is_empty($params['ch_filters'])) {
		foreach ($params['ch_filters'] as $k => $v) {
			...
			// Feature filter
				...
				$fid = intval($k);
				$join .= db_quote(" LEFT JOIN ?:product_features_values as ch_features_$fid ON ch_features_$fid.product_id = products.product_id ");
	...
	if (!empty($params['tx_features'])) {
		foreach ($params['tx_features'] as $k => $v) {
			if (fn_string_no_empty($v)) {
				...
				$condition .= db_quote(" AND tx_features_$fid.value LIKE ?l ", "%" . trim($v) . "%");
			}
		}
	}
	...
	if (!empty($params['cid'])) {
		...
		$params['parent'] = (int)db_get_field("SELECT parent_id FROM ?:categories WHERE category_id = ?i", $params['cid']);
		$fields[] = 'products_categories.position';
		$params['extend'][] = 'categories';
		$condition .= db_quote(" AND ?:categories.category_id IN (?n)", $cids);
	}

	if(is_array($params['features_hash']) && $_REQUEST['category_id']){
			$_ids = db_get_fields("SELECT a.category_id FROM ?:categories as a LEFT JOIN ?:categories as b ON b.category_id IN (?n) WHERE a.id_path LIKE CONCAT(b.id_path, '/%')", $_REQUEST['category_id']);

			$cids = fn_array_merge($_REQUEST['category_id'], $_ids, false);
	...
	if (!empty($params['price_from']) && !empty($params['price_to'])) {
	    foreach ($params['price_from'] as $price_id=>$price_from){
	      if(fn_is_numeric($price_from)){
		$where_price[] = db_quote(' (prices.price >= ?d AND prices.price <= ?d) ', fn_convert_price(trim($price_from)), fn_convert_price(trim($params['price_to'][$price_id])));
	      }
	    }
		$condition .= ' AND ('.implode(' OR ', $where_price).')';
	}
      } else {
	...
	//added by ankur for the cod option
	if (!empty($params['is_cod'])) {
		$condition .= db_quote(' AND products.is_cod = ?s', $params['is_cod']);
	}
	//code end
	//added by ankur for the price see inside
	if (isset($params['price_see_inside']) && $params['price_see_inside']!='') {
		$condition .= db_quote(' AND products.price_see_inside = ?s', $params['price_see_inside']);
	}
	//code end
	//added by ankur for the special_offer_badge
	if (isset($params['special_offer_badge']) && $params['special_offer_badge']!='') {
		
		if($params['special_offer_badge']==1)
		{
			$condition .= db_quote(' AND (products.special_offer_badge = ?s or products.special_offer_text!="" or products.special_offer!="")', $params['special_offer_badge']);
		}
		else
		{
			$condition .= db_quote(' AND (products.special_offer_badge = ?s or products.special_offer_text="" or products.special_offer="")', $params['special_offer_badge']);
	...
	//code end
	//added by ankur for the deal_inside_badge
	if (isset($params['deal_inside_badge']) && $params['deal_inside_badge']!='') {
		$condition .= db_quote(' AND products.deal_inside_badge = ?s', $params['deal_inside_badge']);
	}
	//code end
	//added by ankur for the  freebee_inside
	if (isset($params['freebee_inside']) && $params['freebee_inside']!='') {
		$condition .= db_quote(' AND products.freebee_inside = ?s', $params['freebee_inside']);
	}
	//code end
	//added by ankur for the  search_words
	if (isset($params['search_words']) && $params['search_words']!='') {
		if($params['search_words']==1)
		{
			$condition .= db_quote(' AND descr1.search_words != ""');
		}
		else
		{
			$condition .= db_quote(' AND descr1.search_words = ""');
	...
	//code end
	//added by ankur for the seo_entered
	if (isset($params['seo_entered']) && $params['seo_entered']!='') {
		if($params['seo_entered']==1)
		{
			$condition .= db_quote(' AND descr1.page_title != ""');
		}
		else
		{
			$condition .= db_quote(' AND descr1.page_title = ""');
	...
	//code end
	//added by ankur for the  sold out
	if (isset($params['sold_out']) && $params['sold_out']!='') {
		
		
		if($params['sold_out']==1)
		{
	...
	if (in_array('product_name', $params['extend'])) {
		...
		// Edited by Sudhir dt 13th octo 2012 to optimize multi select search query
		if (($params['dispatch'] != 'products.search') AND ($params['dispatch'] != 'categories.view')) {
			$join .= db_quote(" LEFT JOIN ?:product_descriptions as descr1 ON descr1.product_id = products.product_id ");
		} else {
			$join .= db_quote(" INNER JOIN ?:product_descriptions as descr1 ON descr1.product_id = products.product_id ");
		} // Edited by Sudhir dt 13th octo 2012 to optimize multi select search query end here
	}
	...
	if (in_array('prices2', $params['extend'])) {
		...
		if (($params['dispatch'] != 'products.search') AND ($params['dispatch'] != 'categories.view')) {
			...
			$join .= " LEFT JOIN ?:product_prices as prices_2 ON prices.product_id = prices_2.product_id AND prices_2.price < prices.price " . $price_usergroup_cond_2;
	...
	if (($params['dispatch'] != 'products.search') AND ($params['dispatch'] != 'categories.view')) {
        ...
        if(isset($params['market_id']) && $params['market_id'] != ''){
                $join .= "Inner join clues_company_badge_answer as ccab on ccab.company_id=companies.company_id and ccab.badge_id='2' and ccab.question_id='".$params['market_id']."'";
        }
	// get popularity
	...
	if (!empty($params['get_subscribers'])) {
        ...
        if (!empty($params['top_products'])){
            
            /*$join .= "left join cscart_order_details od on od.product_id = products.product_id
inner join cscart_orders o on o.order_id = od.order_id and o.status not in ('F','I','N','D')"; */
            
            $join .= "left join cscart_product_popularity pp on pp.product_id = products.product_id
                       ";
        }

	$sorting = $sortings[$params['sort_by']] .' '.$directions[$params['sort_order']];
	if(empty($_REQUEST['sort_by']) && $params['sort_by']=="popularity") {
        $sorting = "products.boost_index asc,".$sorting;
	}
	// -- \SORTINGS --
	// Reverse sorting (for usage in view)
	//$params['sort_order'] = ($params['sort_order'] == 'asc') ? 'desc' : 'asc';
	...
	if (!empty($items_per_page)) {
       ...
       //change by ankur to get the item range according to usergroup
	   if($auth['user_id']!=1 && AREA == 'A')
	   {
	   		$limit_arr=fn_get_user_max_limit($auth['usergroup_ids']);
			if(!empty($limit_arr))
			{
				$limit = fn_paginate($params['page'], 0, $items_per_page, true,$limit_arr);
			}
			else
			{
	...
	if(AREA == 'A'){
		...
		//added by ankur to get the feature index,deals index and product popularity
		if(!defined("COMPANY_ID") && MODE=='manage' && CONTROLLER=='products')
		{
			$fields[]="products.feature_index";
			$fields[]="products.deals_index";
			$fields[]="?:product_popularity.total as pro_popularity";
			$join.=" LEFT JOIN ?:product_popularity on ?:product_popularity.product_id=products.product_id";
		}
		//code end
                if(!empty($params['top_products'])){
                
                $sql_search = "SELECT SQL_CALC_FOUND_ROWS " . implode(', ', $fields) . ",sum(inv.amount) as inv_amount ,if(products.tracking='O',IF(sum(inv.amount)=0 OR sum(inv.amount) IS NULL,0,1),IF(products.amount=0,0,1)) as product_amount_available FROM ?:products as products $join left join cscart_product_options_inventory as inv on inv.product_id=products.product_id  WHERE 1 and companies.company_id=".$_SESSION['auth']['company_id']." group by products.product_id order by pp.bought desc limit ".$_REQUEST['top_products'];
               
                }else{
                
	 	$sql_search = "SELECT SQL_CALC_FOUND_ROWS " . implode(', ', $fields) . ",sum(inv.amount) as inv_amount,if(products.tracking='O',IF(sum(inv.amount)=0 OR sum(inv.amount) IS NULL,0,1),IF(products.amount=0,0,1)) as product_amount_available FROM ?:products as products $join left join cscart_product_options_inventory as inv on inv.product_id=products.product_id  WHERE 1  $condition GROUP BY $group_by ORDER BY if(products.amount=0,0,1) desc,$sorting ";
                
                $total = count(db_get_array("SELECT products.product_id FROM ?:products as products $join left join cscart_product_options_inventory as inv on inv.product_id=products.product_id  WHERE 1  $condition GROUP BY $group_by ORDER BY if(products.amount=0,0,1) desc,$sorting "));
		...
		// query changes for clues blocks changed by Sudhir dt 12th Octo 2013 bigin here
...
//sapna added amount and prod_inv in query
			$select_fields = "products.product_id, descr1.product as product, MIN(prices.price) as price, companies.company as company_name, GROUP_CONCAT(IF(products_categories.link_type = 'M', CONCAT(products_categories.category_id, 'M'), 	products_categories.category_id)) as category_ids, cscart_seo_names.name as seo_name, products.promotion_id, products.list_price, products.deal_inside_badge, products.special_offer_badge, products.price_see_inside, products.special_offer_text,products.amount,sum(inv.amount) as inv_amount,if(products.tracking='O',IF(sum(inv.amount)=0 OR sum(inv.amount) IS NULL,0,1),IF(products.amount=0,0,1)) as product_amount_available , products.freebee_inside, products.timestamp";
			$select_orderby = " if(products.tracking='O',IF(sum(inv.amount)=0 OR sum(inv.amount) IS NULL,0,1),IF(products.amount=0,0,1)) ";
		}elseif($params['sort_by'] == 'popularity' || $params['sort_by'] == 'hotdeals' || $params['sort_by'] == 'newarrivals' || $params['sort_by'] == 'featured'){
			$select_fields = " SQL_CALC_FOUND_ROWS " . implode(', ', $fields) . ",sum(inv.amount) as inv_amount,if(products.tracking='O',IF(sum(inv.amount)=0 OR sum(inv.amount) IS NULL,0,1),IF(products.amount=0,0,1)) as product_amount_available";
			$select_orderby = " if(products.tracking='O',IF(sum(inv.amount)=0 OR sum(inv.amount) IS NULL,0,1),IF(products.amount=0,0,1)) ";
		} else {
			$select_fields = " SQL_CALC_FOUND_ROWS " . implode(', ', $fields) . ",sum(inv.amount) as inv_amount,if(products.tracking='O',IF(sum(inv.amount)=0 OR sum(inv.amount) IS NULL,0,1),IF(products.amount=0,0,1)) as product_amount_available";
			//$select_orderby = " if(products.amount=0,0,1) ";
			$select_orderby = " if(products.tracking='O',IF(sum(inv.amount)=0 OR sum(inv.amount) IS NULL,0,1),IF(products.amount=0,0,1)) ";
		}

		// query changes for clues blocks changed by Sudhir dt 12th Octo 2013 end here
		// changed by Sudhir dt 31st octo 2012 to make this query memcache for customer side start here
			/*if(Registry::get('config.memcache') && $GLOBALS['memcache_status'])
			{
				$memcache = $GLOBALS['memcache'];
				$key = md5("SELECT $select_fields FROM ?:products as products $join left join cscart_product_options_inventory as inv on inv.product_id=products.product_id  WHERE 1 AND products.status='A' $condition GROUP BY $group_by ORDER BY $select_orderby desc,$sorting $limit");
				if($mem_value = $memcache->get($key)){
					$sql_search = $mem_value;			
				} else {
				 	$sql_search = "SELECT $select_fields FROM ?:products as products $join left join cscart_product_options_inventory as inv on inv.product_id=products.product_id  WHERE 1 AND products.status='A' $condition GROUP BY $group_by ORDER BY $select_orderby desc,$sorting $limit";
					$status = $memcache->set($key, $sql_search, MEMCACHE_COMPRESSED, Registry::get('config.memcache_expire_time'));
		                        if(!$status){
		                            $memcache->delete($key);
		                        }
				}
			 	...
			 	$sql_search = "SELECT $select_fields FROM ?:products as products $join left join cscart_product_options_inventory as inv on inv.product_id=products.product_id  WHERE 1 AND products.status='A' $condition GROUP BY $group_by ORDER BY $select_orderby desc,$sorting $limit";
			//}
		// changed by Sudhir dt 31st octo 2012 to make this query memcache for customer side end here
	}
    	//echo "<br><br><br><br>".db_process($sql_search);

	//echo 'pp=<pre>'; print_r($params);echo '</pre>';
	if(!empty($params['cid'])) $show_product_listing = fn_check_category_meta($params['cid']);

	if(empty($params['features_hash']) && !empty($params['item_ids']) && Registry::get('config.homepage_solr') && AREA=='C') {		
		$products = fn_get_products_solr($params);
	} 
        elseif (empty($params['features_hash']) && $params['parent'] != 0 && $show_product_listing != 'N' && Registry::get('config.category_solr') && empty($params['item_ids']) && AREA=='C') {      //if not metacategory then get data for the category.
                //$products = fn_get_products_cat_solr($params,$items_per_page,$cids);
                $products = fn_get_solr_categories($params,$items_per_page,$cids);
        }
        elseif(empty($params['features_hash']) && $params['parent'] == 0 && Registry::get('config.root_category_solr') && empty($params['item_ids']) && AREA == 'C' ){
            ...
            $products = fn_get_products_cat_solr($params,$items_per_page,$cids);               
                ...
                $products = db_get_array($sql_search);
        }
        //$cnt_for_all = db_get_found_rows();
	$sql_for_total_count = "SELECT count(distinct products.product_id) FROM ?:products as products $join left join cscart_product_options_inventory as inv on inv.product_id=products.product_id  WHERE 1 AND products.status='A' $condition";
	$cnt_for_all = db_get_field($sql_for_total_count);

	
	//HPRAHI Code start
	if($cnt_for_all==0 && isset($params['cid']) && $params['cid']!=0 && $_REQUEST['dispatch'] == 'products.search' && !is_array($params['features_hash']))
	{
		$params['cid']=0;
		$in_hp = strrpos($sql_search,'AND ?:categories.category_id IN');
		$str1_hp = substr($sql_search,$in_hp);
		$str_hp = substr($sql_search,0,$in_hp);
		$in_hp = stripos($str1_hp,')');
		$str2_hp = substr($str1_hp,$in_hp+1);
		$sql_search_with_no_cat = $str_hp . $str2_hp;
		$products = db_get_array($sql_search_with_no_cat);
		$cnt_for_all = db_get_found_rows();
		if($cnt_for_all>0)
		{
			$_SESSION['changecatidtozero']=1;
		}
		else
		{
			unset($_SESSION['changecatidtozero']);
		}
	}
	else
	{
	...
	if (!empty($items_per_page)) {
		$total = $cnt_for_all;
			if(!empty($limit_arr) && AREA == 'A')
			{      if(!isset($params['top_products'])){ 
				  fn_paginate($params['page'], $total_count, $items_per_page,false,$limit_arr);                              }
			}
			else
			{     if(!isset($params['top_products'])){
                            
                              if(!empty($_SESSION['auth']['company_id'])){
                                  fn_paginate($params['page'], $total_count, $items_per_page);
                              }else{
	...
	fn_view_process_results('products', $products, $params, $items_per_page);
        ...
        //code by anoop to check value added services on product
        if(!empty($products))
        {
            $product_ids = array();
            ...
            $value_added_service_info = fn_check_if_value_added_service_going_on_product($product_ids);
            
            foreach($products as $pkey => $pdata)
            {
                $products[$pkey]['value_added_services'] = $value_added_service_info[$pdata['product_id']];
            }
        }
...
function fn_get_products_sorting($simple_mode = true)
{
	$sorting = array(
		...
		'boost_index' => array('description' => fn_get_lang_var('boost_index'), 'default_order' => 'asc')
	);
...
function fn_get_categories_list($category_ids,$parent_id='',$lang_code = CART_LANGUAGE) //change by ankur to get categories path on the basis of group for the feature
{
	static $max_categories = 10;
	$c_names = array();
	if(!empty($parent_id))
	{
		$category_ids=db_get_field("select categories_path from cscart_product_features where feature_id='".$parent_id."'");
	}
...
function fn_get_billing_categories($company_id)
{       
	$billing_categories = db_get_array("SELECT id,category,default_commision FROM clues_billing_categories where end_date='0000-00-00'");
        $categoryIds = db_get_fields("SELECT billing_category_id FROM clues_billing_companies_commission WHERE company_id= $company_id");
        if(is_array($categoryIds)) {
		foreach($billing_categories as $key => $categories_data) {
			if(in_array($categories_data['id'], $categoryIds)) {
				$billing_categories[$key]['exists'] = true;
			}
		}
	}
        return $billing_categories;
}


function get_products_feature($product_id){//feature by chandan
	$sql ="select distinct(cpf.feature_id), cpf.feature_type, cpf.position, cpf.is_key, cpfd.description, cpfv.variant_id, cpfvd.variant 
			from cscart_product_features_values cpfv
			left join cscart_product_feature_variant_descriptions cpfvd on (cpfv.variant_id=cpfvd.variant_id)
			left join cscart_product_features cpf on (cpfv.feature_id=cpf.feature_id)
			left join cscart_product_features_descriptions cpfd on (cpfv.feature_id=cpfd.feature_id)
			where cpf.status = 'A' and cpf.is_key='1'and cpfv.product_id = '".$product_id."' order by cpf.position limit 0,3";
	$result = db_get_array($sql);
	
	return $result;
}

//function by ankur for product required field validatin
function fn_check_product_required_field($pro_id)
{
	$error=0;
	$error_flag=array();
	$product_data = fn_get_product_data($pro_id, $auth, DESCR_SL, '', true, true, true, true);
	...
	{
		$error=1;
		$error_flag['Category']=1;
	}
	
	if(floor($product_data['price'])==0)
	{
		$error=1;
		$error_flag['Selling Price']=1;
	}
	if(!isset($product_data['main_pair']['detailed']['image_path']) && !isset($product_data['main_pair']['icon']['image_path']))
	{
		$error=1;
		$error_flag['Image']=1;
	}
	...
	{
		$error=1;
		$error_flag['Description']=1;
	}
	if(empty($product_data['product']))
	{
		$error=1;
		$error_flag['Product Name']=1;
	}
	foreach($product_data['product_features'] as $feature_id=>$features)
	{
		if($features['feature_type']!='G' && $features['required_feature']=='Y' )
		{
			if(empty($features['variant_id']))
			{
				$error=1;
				$error_flag[$features['description']]=1;
			}
		}
		else if($features['feature_type']=='G')
		{
			if(!empty($features['subfeatures']))
			{
				foreach($features['subfeatures'] as $sub_fetid=>$sub_feat)
				{
					if($sub_feat['required_feature']=='Y' && empty($sub_feat['variant_id']))
					{
						$error=1;
						$error_flag[$sub_feat['description']]=1;
					}
	...
	if($error==0)
	{
		$error_flag['validation']=1;
		return $error_flag;
	}
	else
	{
		$error_flag['validation']=0;
		return $error_flag;
	}
}
//function end

//function by ankur to get contest data
function fn_get_contest_data($product_id)
{
	
	$sql="select contest_icon_url,contest_message from cscart_products where product_id='".$product_id."'";
	if(Registry::get('config.memcache') && $GLOBALS['memcache_status'])
	{
		$memcache = $GLOBALS['memcache'];
...
function fn_get_user_max_limit($user_group_id)
{
	$conf_limit=Registry::get('config.user_export_count_limit');
	$limit=0;
	
	foreach($user_group_id as $group_id)
	{
		$def_limit=$conf_limit[$group_id];
		if($def_limit>$limit)
		$limit=$def_limit;
	}
	if(!empty($limit))
	{
		if($limit>100)
		{
			$arr_limit1=array(10,20,30,50,100);
			$arr_limit2=range(200, $limit, 100);
			$arr_limit=array_merge($arr_limit1,$arr_limit2);
		}
		else
		{
			$arr_limit=range(10, $limit, 10);
		}
...
function fn_get_product_option_image($pro_id,$pro_option)
{
	$keys=array_keys($pro_option);
	$option_combination=$keys[0].'_'.$pro_option[$keys[0]];
	
	$sql="select combination_hash from cscart_product_options_inventory where product_id='".$pro_id."' and combination='".$option_combination."'";
	$comb_hash=db_get_field($sql);
	$images=fn_get_image_pairs($comb_hash, 'product_option', 'M');
	
	return $images;
}

function fn_get_nrh_categories($market_id) {
	$sql = "select 
		c.category_id, 
		c.parent_id, 
		c.id_path, 
		...
		c.status, 
		cd.category,
		SUBSTRING_INDEX(SUBSTRING_INDEX(c.id_path, '/',2), '/',-1) as pcategory_id
		from 
		cscart_categories as c 
		left join cscart_category_descriptions as cd on cd.category_id = SUBSTRING_INDEX(SUBSTRING_INDEX(id_path, '/',2), '/',-1) 
		left join clues_market_category as mc on mc.nrh_category_id = c.category_id 
		where mc.market_id = ?i group by pcategory_id";
	   return db_get_array($sql, $market_id);
}

function fn_get_nrh_markets($market_id) {
    return db_get_row("SELECT market.id AS market_id, market.name AS name, details.description AS description, details.slug AS slug, details.seo_name AS seo_name
          FROM clues_badge_questions AS market
          LEFT JOIN clues_markets AS details ON market.id = details.market_id
          WHERE market.id =?i ", $market_id);
}

function fn_get_root_category($id_path){
        $res = explode('/',$id_path);
        return $res['0'];
}

function fn_get_array_chunk($array)
{
    $x = array();
    $z =0;
...
function fn_get_selected_product_ids($prod_id)
{
	return db_get_array("SELECT cs.name,cd1.category,vd.variant,concat('images/thumbnails/',floor(if(i.image_id!=0, i.image_id,il.image_id)/1000),'/320/320/',REPLACE(REPLACE(image_path, '\n', ''), '\r', '')) as image_path
		FROM cscart_products cp 
		inner join cscart_seo_names cs on cp.product_id=cs.object_id and cs.type='p'
		INNER JOIN cscart_products_categories cps on cps.product_id = cp.product_id and cps.link_type='M' 
		INNER JOIN cscart_categories c1 ON c1.category_id = cps.category_id AND c1.status in('A','H')
		INNER JOIN cscart_category_descriptions cd1 on cd1.category_id=SUBSTRING_INDEX(SUBSTRING_INDEX(id_path, '/',2), '/',1) 
		LEFT JOIN cscart_images_links il ON il.object_id = cp.product_id and il.object_type = 'product' and il.type = 'M'
		LEFT JOIN cscart_images i ON il.detailed_id = i.image_id
		left JOIN cscart_product_features_values cfv on cfv.product_id = cp.product_id and cfv.feature_id=53 
		left JOIN cscart_product_feature_variant_descriptions vd on vd.variant_id = cfv.variant_id where cp.product_id=$prod_id");
}
