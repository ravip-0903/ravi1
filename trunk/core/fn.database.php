...
{
	$args = func_get_args();
	$is_cache = check_query_for_cache($query,$args);
    $result = "";
	if(Registry::get('config.memcache') && $GLOBALS['memcache_status'] && $is_cache == 'Y')
    {
        $memcache = $GLOBALS['memcache'];
        $key = md5(db_process($query, array_slice($args, 1), true));
        ...
        if(($mem_value = $memcache->get($key)) !== false){
		   ...
		   if ($_result = call_user_func_array('db_query', $args)) {
				...
				$memcache->set($key, $result, MEMCACHE_COMPRESSED, Registry::get('config.memcache_expire_time'));
			}
        }
    }else{
		if ($_result = call_user_func_array('db_query', $args)) {
	...
	if(Registry::get('config.memcache') && $GLOBALS['memcache_status'] && $is_cache == 'Y')
    {
        ...
        if(($mem_value = $memcache->get($key)) !== false){
		   ...
		   if ($_result = call_user_func_array('db_query', $args)) {
				...
				$memcache->set($key, $result, MEMCACHE_COMPRESSED, Registry::get('`.memcache_expire_time'));
			}
        }
    }else{
		if ($_result = call_user_func_array('db_query', $args)) {
...
function db_get_field($query)
{
	...
	$is_cache = check_query_for_cache($query,$args);
        $result = "";
	////echo '<br> Is Cache : '.$is_cache.'##Query : '.$query;
	if(Registry::get('config.memcache') && $GLOBALS['memcache_status'] && $is_cache == 'Y')
    {
        $memcache = $GLOBALS['memcache'];
        $key = md5(db_process($query, array_slice($args, 1), true));
        ...
        if(($mem_value = $memcache->get($key)) !== false){
		   ...
		   if ($_result = call_user_func_array('db_query', $args)) {
				$row = driver_db_fetch_row($_result);
				driver_db_free_result($_result);
				if($row !== false){
                                    $result = $row;
                                }
...
/**
 * Delete a query cached in memcache
 */
function memcache_delete_query($query)
{
    $args = func_get_args();
...
function db_get_hash_single_array($query, $params)
{
	...
	{
            $memcache = $GLOBALS['memcache'];
            $memkey = md5(db_process($query, array_slice($args, 1), true));
            $GLOBALS['memcache_key'][] = $memkey;
            if (isset($_REQUEST['clean']) 
        		&& $_REQUEST['clean'] == 'scrp' 
				&& !empty($_SESSION['auth']['user_id']) 
				&&  in_array($_SESSION['auth']['user_id'], Registry::get('config.memcache_reset_user_id'))) {
            ...
            if(($mem_value = $memcache->get($memkey)) !== false){
                $result = $mem_value;
                $fetch_from_db = false;
            }
	}
        ...
        if($fetch_from_db){
            if ($_result = call_user_func_array('db_query', $args)) {
                    ...
                    if(Registry::get('config.memcache') && $GLOBALS['memcache_status'] && $is_cache == 'Y')
                    {
                        $memcache->set($memkey, $result, MEMCACHE_COMPRESSED, Registry::get('config.memcache_expire_time'));
                    }
            }
        }
...
function db_query($query)
{
	...
	$chk_query = strtolower(trim($query));	

	if (Registry::get('config.readonly_browser') == 'TRUE'){
		if((substr($chk_query,0,7) == 'replace') || (substr($chk_query,0,6) == 'insert') || (substr($chk_query,0,6) == 'update') || (substr($chk_query,0,6) == 'delete')){
			$result = false;
		}else{
...
//				$pattern = db_str_replace($ph, $data[$k], $pattern, $offset);
				$pattern = db_str_replace($ph, str_replace('?:', Registry::get('runtime.db_prefixes.' . $dbc_name), $data[$k]), $pattern, $offset);
			}
		}
	}
	...
	if (!empty($arr)) {
		foreach ($arr as $val) {
			//$conditions[] = db_quote("FIND_IN_SET(?i, $set)", $val);
                          $conditions[] = db_quote("$set LIKE '%$val%'");
		}
	}
...
function db_prepare(){
    // Connect to database
    Registry::set('runtime.db.prepare.called', true);
    if(!Registry::get('config.db_host')){
        throw new Exception('Config : mysql db settings not defined in config');
    }
...
function check_query_for_cache($query, $args){
	$execlude_list = Registry::get('config.query_execlude_memcache');
	$regex_pattern = "'/".implode('|',$execlude_list)."/i'";
	$regex_pattern = preg_replace('/[^a-zA-Z0-9.,\/|\_=]/','', $regex_pattern);
	$is_cache = 'Y';
	$query = db_process($query, array_slice($args, 1), true);
