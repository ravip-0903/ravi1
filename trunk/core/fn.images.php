...
{
	// Added By Sudhir dt 20th Dec 2012 to validate special character in image name bigin here 
	$image_data['name'] = preg_replace("/[^a-zA-Z0-9.]/i", "", $image_data['name']);
	$imgname = explode('.',$image_data['name']);	
	$image_data['name'] = $imgname[0].'_'.time().'.'.$imgname[1];
	// Added By Sudhir dt 20th Dec 2012 to validate special character in image name end here 
	...
	if (!empty($fd)) {
		...
		if ($image_path != $image_data['name'] && empty($rev_data)) {
			// fn_delete_file(DIR_IMAGES . $images_path . $image_path); //Commented by Sudhir dt 08 july 2013 to improve server performance
		}
		...
		//fn_delete_file_thumbnails($image_data['name']); // commented by Sudhir dt 10th Apr 2013 to fix slow issue at admin update product
		...
		if (@fn_rename($image_data['path'], DIR_IMAGES . $images_path . $image_data['name']) == false) {
			...
			// Added By Sudhir dt 14th nov 2012 to upload images at image server bigin here 
			/*$local = Registry::get('config.loc_prd_img'). $images_path . $image_data['name'];
			...
			$local = Registry::get('config.loc_img'). $images_path.$image_data['name'];
			$remote =  Registry::get('config.remote_img');
			$parameter = Registry::get('config.rsync_parameter');
			$rsyn = exec("rsync $parameter $local $remote &");

			// Added By Sudhir dt 14th nov 2012 to upload images at image server end here 
...
function fn_delete_image($image_id, $pair_id, $object_type = 'product', $rev_data = array())
{
	...
	// Commented By Sudhir dt 08 july 2013 to improve web server performance bigin here
	...
	if (empty($dir_content)) {
		fn_rm($path);
	}*/ // Commented By Sudhir dt 08 july 2013 to improve web server performance end here
		...
		if ($get_icon == true) {
			$join_cond = "?:images_links.image_id = ?:images.image_id";
                    ...
                    {       
                        ...
                        $value = "SELECT ?:images_links.*, ?:images.image_path, ?:common_descriptions.description AS alt, ?:images.image_x, ?:images.image_y, ?:images.image_id as images_image_id FROM ?:images_links USE INDEX(object_id) LEFT JOIN ?:images ON". $join_cond ."LEFT JOIN ?:common_descriptions ON ?:common_descriptions.object_id = ?:images.image_id AND ?:common_descriptions.object_holder = 'images' AND ?:common_descriptions.lang_code = ?s WHERE ?:images_links.object_type = ?s AND ?:images_links.type = ?s". $cond .$lang_code .$object_type.$pair_type ;
                        $key = md5($value);
                        ...
                        if($mem_value = $memcache->get($key)){
                           $icon_pairs = $mem_value;        
                         }else{
                            $icon_pairs = db_get_array("SELECT ?:images_links.*, ?:images.image_path, ?:common_descriptions.description AS alt, ?:images.image_x, ?:images.image_y, ?:images.image_id as images_image_id FROM ?:images_links USE INDEX(object_id) LEFT JOIN ?:images ON $join_cond LEFT JOIN ?:common_descriptions ON ?:common_descriptions.object_id = ?:images.image_id AND ?:common_descriptions.object_holder = 'images' AND ?:common_descriptions.lang_code = ?s WHERE ?:images_links.object_type = ?s AND ?:images_links.type = ?s $cond", $lang_code, $object_type, $pair_type);
                            $status = $memcache->set($key, $icon_pairs, MEMCACHE_COMPRESSED, Registry::get('config.memcache_expire_time')); // or die ("Failed to save data at the server");
                        if(!$status){
                            $memcache->delete($key);
                        }
		...
		if ($get_detailed == true) {
			$join_cond = db_quote("?:images_links.detailed_id = ?:images.image_id");
                    ...
                    {       
                        ...
                        if($mem_value = $memcache->get($key)){
                           $detailed_pairs = $mem_value;        
                         }else{
                            $detailed_pairs = db_get_array("SELECT ?:images_links.*, ?:images.image_path, ?:common_descriptions.description AS alt, ?:images.image_x, ?:images.image_y, ?:images.image_id as images_image_id FROM ?:images_links USE INDEX(object_id) LEFT JOIN ?:images ON $join_cond LEFT JOIN ?:common_descriptions ON ?:common_descriptions.object_id = ?:images.image_id AND ?:common_descriptions.object_holder = 'images' AND ?:common_descriptions.lang_code = ?s WHERE ?:images_links.object_type = ?s AND ?:images_links.type = ?s $cond", $lang_code, $object_type, $pair_type);
                            $status = $memcache->set($key, $detailed_pairs, MEMCACHE_COMPRESSED, Registry::get('config.memcache_expire_time')); // or die ("Failed to save data at the server");
                        if(!$status){
                            $memcache->delete($key);
                        }
...
function fn_resize_image($src, &$dest, $new_width = 0, $new_height = 0, $make_box = false, $bg_color = '#ffffff', $save_original = false)
{
	...
	if (file_exists($src) && !empty($dest) && (!empty($new_width) || !empty($new_height)) && extension_loaded('gd')) {
		...
		// Commented by Sudhir dt 08 july 2013 to improve server performance bigin here
		/*if (!$save_original) {
			fn_rm($src);
		}*/
		// Commented by Sudhir dt 08 july 2013 to improve server performance end here
		...
		@chmod($dest, DEFAULT_FILE_PERMISSIONS);
			...
			/*$remote =  Registry::get('config.remote_prd_img');
			$parameter = Registry::get('config.rsync_parameter');
			$rsyn = exec("rsync $parameter $dest $remote &");*/

			 $img_path = explode('/images/',$dest);
                         $local = Registry::get('config.loc_img').$img_path[1];
                         $remote =  Registry::get('config.remote_img');
                         $parameter = Registry::get('config.rsync_parameter');
...
function fn_get_image_size($file)
{
	...
	// Added by Sudhir dt 27th octo 2012 to not allow image that has size greater than 200kb
	/*$FileSize = exif_read_data($file);
	if($FileSize['$FileSize'] > 200000){
		return false;
	}*/ // Added by Sudhir dt 27th octo 2012 to not allow image that has size greater than 200kb end here
...
function fn_attach_image_pairs($name, $object_type, $object_id = 0, $lang_code = CART_LANGUAGE, $object_ids = array (), $parent_object = '', $parent_object_id = 0)
{
	...
	//$x = fn_update_image_pairs($icons, $detailed, $pairs_data, $object_id, $object_type, $object_ids, $parent_object, $parent_object_id, true, $lang_code);//die();return $x;
}
...
function fn_generate_thumbnail($image_path, $width, $height = 0, $make_box = false)
{
 ...
 //Hprahi codes
$th_path1 = htmlspecialchars_decode(Registry::get('config.internal_images_host')."/images/thumbnails/".$filename);
$real_path = htmlspecialchars_decode(Registry::get('config.internal_images_host')."/".$_path);

//$file_headers = @get_headers($th_path1);
//if(strpos($file_headers[0], '404 Not Found')) {

$handle = curl_init($th_path1);
curl_setopt($handle,  CURLOPT_RETURNTRANSFER, TRUE);
$response = curl_exec($handle);
$httpCode = curl_getinfo($handle, CURLINFO_HTTP_CODE);
if($httpCode == 404) {

  $th_path1 = htmlspecialchars_decode(Registry::get('config.internal_images_server')."/images/thumbnails/".$filename);
  $real_path = htmlspecialchars_decode(Registry::get('config.internal_images_server')."/".$_path);
...
function fn_get_img_path($images, $width, $height){
	
	if(isset($images['detailed']['image_path'])){
	    $pth = explode("/", $images['detailed']['image_path']);
	}elseif(isset($images['icon']['image_path'])){
	    $pth = explode("/", $images['icon']['image_path']);
	}else{
		return Registry::get('config.ext_images_host').'/images/no_image.gif';
	}
	...
	foreach($pth as $val){
        ...
        $img_path = Registry::get('config.ext_images_host')."/images/thumbnails/". $path[1]. "/".$width."/".$height."/".preg_replace("/[^a-zA-Z0-9.]/i", "", $path[0]);
