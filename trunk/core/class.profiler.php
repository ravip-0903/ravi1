...
//
// $Id: class.profiler.php 12865 2011-07-05 06:57:22Z 2tl $
//
	...
	static function checkpoint($name)
	{
		...
		self::$checkpoints[$name] = array(
			...
			'queries' => count(self::$queries),
                ...
                static $last_metric;
                $logmetrics['execution_point'] = $name;
                $logmetrics = array_merge($logmetrics, self::$checkpoints[$name]);
                if(count($last_metric) > 0){
                    $logmetrics['time'] = round($logmetrics['time'] - $last_metric['time'],4);
                }
                else{
                    $logmetrics['time'] = 0;
                }
                $last_metric = self::$checkpoints[$name];
                LogMetric::dump_log(array_keys($logmetrics), array_values($logmetrics));
	}
	...
	static function display()
	{
		...
		if (defined('PROFILER_SQL')) {
			...
			echo '</ul>';
                ...
                $memcache_key = implode(',',$GLOBALS['memcache_key']);
                
                echo '<br />- Memcache Keys: '.$memcache_key; 
