...
{ 
	if(!Registry::get('config.log_events')){
		return true;	
	}
	$update = false;
	$content = array();
	...
	fn_set_hook('save_log', $type, $action, $data, $user, $content, $event_type);
        ...
        $log_metrics = $content;
        $log_metrics['user_id'] = $user_id;
        $log_metrics['type'] = $type;
        $log_metrics['action'] = $action;
        $log_metrics['event_type'] = $event_type;
        if($data['backtrace'] != ''){
            $log_metrics['backtrace'] = $data['backtrace'];
        }
        
	$content = serialize($content);
	if ($update) {
		$log_metrics['cscart_logs_id'] = $update;
		...
		$log_metrics['cscart_logs_id'] = $log_id;
	}
        LogMetric::dump_log(array_keys($log_metrics), array_values($log_metrics));
