	...
	<tr>
		<td style="background-color: #ffffff; border: 1px solid #e6e6e6; margin: 0px auto 0px auto; padding: 0px 44px 0px 46px; text-align: left;">
			...
			<tr>
                ...
                {if $manifest.Mail_logo.vendor}
                <img src="{if $manifest.Mail_logo.vendor}http://shopclues.com/images/{else}http://shopclues.com/images/{/if}{if $manifest.Mail_logo.vendor}{$manifest.Mail_logo.filename}{/if}" width="{$manifest.Mail_logo.width}" height="{$manifest.Mail_logo.height}" border="0" alt="{$manifest.Mail_logo.alt}" />
                {else}
                	<h2>{$company_name}
                {/if}
                </td>
			...
			{hook name="orders:items_list_row"}
				...
				<tr>
					...
					<td style="padding: 5px 10px; background-color: #ffffff; text-align: right; font-size: 12px; font-family: Arial;">{if $oi.extra.exclude_from_calculate}{$lang.free}{else}{include file="common_templates/price.tpl" value=$oi.original_price}{/if}</td>                   {*sapna*}{if $order_info.parent_order_id =='0'}
					{if $order_info.use_discount}
