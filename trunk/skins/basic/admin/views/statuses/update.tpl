{* $Id: update.tpl 12394 2011-05-06 10:46:19Z alexions $ *}
...
<fieldset>
    ...
    <div class="form-field">
		<label for="customer_facing_name_{$st}" class="cm-required">{$lang.cust_name}:</label>
		<input type="text" size="70" id="customer_facing_name_{$st}" name="status_data[customer_facing_name]" value="{$status_data.customer_facing_name}" class="input-text-large main-input" />
	</div>
	...
	<!--Changes by Sudhir dt 29th octo 2012 to define status name with four char bigin here -->
	{if $mode != "add"}
				<input type="hidden" name="status_data[status]" value="{$status_data.status}" />
				<span>{$status_data.status}</span>
	{else}
				<input type="text" maxlength="4" size="4" id="description_{$st}" name="status_data[status]" value="{$status_data.status}" class="input-text" />
				<span>{$status_data.status}</span>
	{/if}
	<!-- Changes by Sudhir dt 29th octo 2012 to define status name with four char end here -->
		</div>
