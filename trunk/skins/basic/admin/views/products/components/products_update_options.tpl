{* $Id: products_update_options.tpl 12544 2011-05-27 10:34:19Z bimib $ *}
		...
		<div class="form-field">
			...
			<select name="global_option[id]" id="global_option_id">
				{foreach from=$global_options item="option_" key="option_id"}
					<option value="{$option_id}">{$option_.option_name}{if $option_.company_id} ({$lang.vendor}: {$option_.company_id|fn_get_company_name}){/if}{if  $option_.staff_notes!=''}-{$option_.staff_notes}{/if}</option>
				{/foreach}
			</select>
		</div>
