...
<div id="content_detailed">
    ...
    <div class="form-field">
		<label for="billing_category" class="cm-required">Billing Category:</label>
		<select name="category_data[billing_category]" id="billing_category" style="width:100px;">
        <option value="">--Select--</option>
        {assign var="cnt" value=1}
        {assign var="billing_category" value=$cnt|fn_hp_get_billing_cat}
        {foreach from=$billing_category item="billing_category"}
        <option value="{$billing_category.id}" {if $category_data.billing_category == $billing_category.id} selected = "selected" {/if}>
	 ...
	 <!--Modified by clues dev to add mark as new -->
      <div class="form-field">
            ...
            <input type="radio" {if $category_data.show_as_new == "Y"}checked="checked"{/if} id="show_as_new" value="Y" name="category_data[show_as_new]" />Yes
            <input type="radio" {if $category_data.show_as_new == "N"}checked="checked"{/if} id="show_as_new" value="N" name="category_data[show_as_new]" />No
        </div>
      <!--Modified by clues dev  to add mark as new -->
      
	  <!--Modified by clues dev to add min deal percentage -->
      <div class="form-field">
            <label for="min_deal_percent">Minimum Deals Percent:</label>
            <input type="text" id="min_deal_percent" value="{$category_data.min_deal_percent}" name="category_data[min_deal_percent]" size="2" maxlength="2" />%
        </div>
      <!--Modified by clues dev  to add min deal percentage -->
      
      
      {if !"COMPANY_ID"|defined}
      	<!--Modified by clues dev to add is meta category -->
     	<div class="form-field">
            <label for="is_meta">{$lang.is_meta_category}:</label>
            <input type="radio" {if $category_data.is_meta == "Y"}checked="checked"{/if} id="is_meta" value="Y" name="category_data[is_meta]" />Yes
            <input type="radio" {if $category_data.is_meta == "N"}checked="checked"{/if} id="is_meta" value="N" name="category_data[is_meta]" />No
        </div>
        
        <div class="form-field">
            <label for="show_feature">{$lang.show_feature}:</label>
            <input type="radio" {if $category_data.show_feature == "Y"}checked="checked"{/if} id="show_feature" value="Y" name="category_data[show_feature]" />Yes
            <input type="radio" {if $category_data.show_feature == "N"}checked="checked"{/if} id="show_feature" value="N" name="category_data[show_feature]" />No
        </div>
        
        <div class="form-field">
            <label for="show_product_listing">{$lang.show_product_listing}:</label>
            <input type="radio" {if $category_data.show_product_listing == "Y"}checked="checked"{/if} id="show_product_listing" value="Y" name="category_data[show_product_listing]" />Yes
            <input type="radio" {if $category_data.show_product_listing == "N"}checked="checked"{/if} id="show_product_listing" value="N" name="category_data[show_product_listing]" />No
        </div>
        
        <div class="form-field">
            <label for="show_to_merchant">{$lang.show_to_merchant}:</label>
            <input type="radio" {if $category_data.show_to_merchant == "Y"}checked="checked"{/if} id="show_to_merchant" value="Y" name="category_data[show_to_merchant]" />Yes
            <input type="radio" {if $category_data.show_to_merchant == "N"}checked="checked"{/if} id="show_to_merchant" value="N" name="category_data[show_to_merchant]" />No
        </div>
	 <div class="form-field">
            <label for="show_image_on_catalog">{$lang.show_image_on_catalog}:</label>
            <input type="radio" {if $category_data.show_image_on_catalog == "Y"}checked="checked"{/if} id="show_image_on_catalog" value="Y" name="category_data[show_image_on_catalog]" />Yes
            <input type="radio" {if $category_data.show_image_on_catalog == "N"}checked="checked"{/if} id="show_image_on_catalog" value="N" name="category_data[show_image_on_catalog]" />No
        </div>
      <!--Modified by clues dev  to add is meta category -->	
	 <div class="form-field">
            <label for="show_seasonal_image_path">{$lang.show_seasonal_image_path}:</label>
		<input type="text" name="category_data[show_seasonal_image_path]" id="show_seasonal_image_path" size="55" value="{$category_data.show_seasonal_image_path}" class="input-text-large main-input" />
        </div>
