{* $Id: extra_list.post.tpl 12143 2011-03-31 12:02:30Z subkey $ *}
...
<tr {$_class}>
	<td>
        ...
        {/if}
        {if $controller=='orders' && $mode=='details'}
         {if $gift.gift_cert_code}
		<p>{$lang.code}:&nbsp;<a href="{"gift_certificates.update?gift_cert_id=`$gift.gift_cert_id`"|fn_url}">{'/[^a-z -\s]/'|preg_replace:'X':$gift.gift_cert_code}</a></p>
		{/if}
        {else}
		{if $gift.gift_cert_code}
           {if AREA=='A'}
			<p>{$lang.code}:&nbsp;{if !"COMPANY_ID"|defined}<a href="{"gift_certificates.update?gift_cert_id=`$gift.gift_cert_id`"|fn_url}">{$gift.gift_cert_code}</a>{else}{'/[^a-z -\s]/'|preg_replace:'X':$gift.gift_cert_code}{/if}</p>
            {else}
