{* $Id: userlog.tpl 12143 2011-03-31 12:02:30Z subkey $ *}
...
<tr>
	...
	<th width="35%">&nbsp;&nbsp;{$lang.reason}</th>
    <th width="10%">&nbsp;&nbsp;Change_Id</th>
    <th width="10%">&nbsp;&nbsp;Expires On</th>
    <th width="10%">Order Payment History</th>
    <th width="10%">&nbsp;&nbsp;Ref_Change_Id</th>
	<th>{$lang.clues_bucks_type}</th>
</tr>
...
<tr {cycle values="class=\"table-row\", "}>
    ...
    <td>{$ul.change_id}</td>
    <td>{$ul.expire_on}(bal.{$ul.balance})</td>
    <td><div style="width:150px;overflow:auto">{$ul.order_payment_history}</div></td>
    <td>{$ul.ref_change_id}</td>
    <td>{$ul.name}</td>
		<!-- Removed the delete link of clues bucks
	<td class="nowrap">
...
{if $userlog}
	<div class="buttons-container buttons-bg">
		<!-- Delete button removal from clues bucks
...
<fieldset>
	<div class="form-field">
		<label for="clues_bucks_type" class="cm-required">{$lang.clues_bucks_type}:</label>
		<select name="reason[clues_bucks_type]" id="clues_bucks_type" />
		<option value="">{$lang._select_bucks_type}</option>
			{foreach from=$bucks_types item="clues_bucks_type"}
				<option value="{$clues_bucks_type.id}">{$clues_bucks_type.name}</option>
			{/foreach}
		</select>
	</div>
	<div class="form-field">
	...
	<div class="form-field">
		<label for="reason_reason" class="cm-required">{$lang.reason}:</label>
    ...
    <div class="form-field">
    	<label for="expire_on">Expires On(days)</label>
        <input type="text" value="" name="expires_on" id="expires_on" />
    </div>
</fieldset>
</div>
