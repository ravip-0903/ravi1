...
<div class="mobile-title" itemprop="name">{$product.product|unescape}</div>
        ...
        {assign var="th_size" value="30"}
                ...
                {include file="common_templates/image.tpl" isMobileSlider=true obj_id="`$product.product_id`_`$image_id`" images=$image_pair_var show_detailed_link=false object_type="detailed_product" show_thumbnail="Y" image_width=$settings.Thumbnails.product_details_thumbnail_width image_height=$settings.Thumbnails.product_details_thumbnail_height rel="preview[product_images]" wrap_image=true}
                ...
                {foreach from=$product.image_pairs item="image_pair"}
                    {if $image_pair}
                        ...
                        {/if}
                        {include file="common_templates/image.tpl" isMobileSlider=true images=$image_pair show_detailed_link=false object_type="detailed_product" show_thumbnail="Y" detailed_link_class="hidden" obj_id="`$product.product_id`_`$image_id`" image_width=$settings.Thumbnails.product_details_thumbnail_width image_height=$settings.Thumbnails.product_details_thumbnail_height rel="preview[product_images]" wrap_image=true}
                    {/if}
                {/foreach}
        </div>
<div class="jcarousel-prev jcarousel-prev-horizontal jcarousel-prev-disabled jcarousel-prev-disabled-horizontal" disabled="true" style="display: block; top: 170px;"></div>
    ...
    {/if}
        ...
        <div class="gifting_notification">
        </div>
...
function SetCookieProd(){
         var cookie_product_ids = ReadCookie('recently_viewed_products_history_cookie');
         var product_id= {/literal}"{$product.product_id}"{literal};
         var product_ids = product_id+','+cookie_product_ids;
         var cookieName = 'recently_viewed_products_history_cookie';
         var cookieValue = [product_ids];
