...
<div class="slider_manualdeals jcarousel-skin-tango">
    <ul id="scroller_{$block.block_id}">
            {foreach from=$products item="product" name="productlisting"}

                <li style="{if $block.properties.number_of_products_to_show == "3"} margin-right:47px; {/if}
...
if({/literal}{$config.isResponsive}{literal} && $(window).width()<801){
			    ...
			    jQuery_1_10_2("#scroller_{/literal}{$block.block_id}{literal}").owlCarousel(optionsOwl);

    jQuery_1_10_2("#scroller_{/literal}{$block.block_id}{literal}").parent().find(".jcarousel-next").click(function(){
        var owl = jQuery_1_10_2(this).parent().find("#scroller_{/literal}{$block.block_id}{literal}");
        owl.trigger('owl.next');
    });
    jQuery_1_10_2("#scroller_{/literal}{$block.block_id}{literal}").parent().find(".jcarousel-prev").click(function(){
        var owl = jQuery_1_10_2(this).parent().find("#scroller_{/literal}{$block.block_id}{literal}");
        owl.trigger('owl.prev');
                        ...
                        $("#scroller_{/literal}{$block.block_id}{literal}").jcarousel({
                            size: {/literal}{$products|count}{literal},
                            scroll:1,
                            item_count: {/literal}{$products|sizeof}{literal}
                        });
                    }
</script>
{/literal}
