...
{if $pagination.total_pages > 1}
	...
	<!--	{$lang.navi_pages}:&nbsp;&nbsp; -->
		...
		{if $pagination.prev_page}
			<a name="pagination" href="{"`$index_script`?`$qstring`&amp;page=`$pagination.prev_page`"|fn_url}" rel="{$pagination.prev_page}" class="cm-history {$ajax_class}" rev="{$id}">{$lang.previous_page}</a>
		{else}
                    <strong style="display: inline!important;background-color: #ccc!important;color: #FFFFFF!important;" class="pagination-selected-page">{$lang.previous_page}</strong>
                {/if}
		...
		{foreach from=$pagination.navi_pages item="pg"}
			...
			{else}
				<strong class="pagination-selected-page" style="display: inline!important;">{$pg}</strong>
			{/if}
		{/foreach}
		...
		{/if}
			...
			<a name="pagination" href="{"`$index_script`?`$qstring`&amp;page=`$pagination.next_page`"|fn_url}" rel="{$pagination.next_page}" class="cm-history {$ajax_class}" rev="{$id}">{$lang.next_page}</a>
		{else}
                    <strong style="display: inline!important;background-color: #ccc!important;color: #FFFFFF!important;" class="pagination-selected-page">{$lang.next_page}</strong>		
                {/if}
