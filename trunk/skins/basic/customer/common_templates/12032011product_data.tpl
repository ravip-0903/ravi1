...
{capture name="name_`$obj_id`"}
	{if $show_name}
		...
		{if $hide_links}<strong>{else}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title{if $product.free_shipping == 'Y'} free-shipping{/if}">{/if}{$product.product|unescape}{if $hide_links}</strong>{else}</a>{/if}
	{elseif $show_trunc_name}
		{if $hide_links}<strong>{else}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title{if $product.free_shipping == 'Y'} free-shipping{/if}" title="{$product.product|strip_tags}">{/if}{$product.product|unescape|truncate:45:"...":true}{if $hide_links}</strong>{else}</a>{/if}
	{/if}
{/capture}
{if $no_capture}
...
{if $show_product_amount && $product.is_edp != "Y" && $settings.General.inventory_tracking == "Y"}
	<span class="cm-reload-{$obj_prefix}{$obj_id}" id="product_amount_update_{$obj_prefix}{$obj_id}">		
       ...
       {* [andyye] *}
		{if isset($addons.sdeep.free_shipping_html) && $addons.sdeep.free_shipping_html && isset($product.free_shipping) && $product.free_shipping == 'Y'}
