{* $Id: step_profile_fields.tpl 10737 2010-09-29 08:42:05Z klerik $ *}
...
<div class="step-complete-wrapper clear">
	...
	{else}
		{if $text}
			<strong class="float-left" style="color:#000;">{$text}: &nbsp;</strong>
		{/if}
		...
		{if $user_country}<p class="no-padding" style="display:inline;">{$user_country}</p>{/if}
        <span><a href="index.php?dispatch=checkout.checkout&edit_step=step_two{if $cart.multiple_shipping_addresses==1}&change_billing=Y{/if}"
									      title="{if $cart.multiple_shipping_addresses==1}Change Billing Address{else}Change Shipping Address{/if}">Change</a></span>
