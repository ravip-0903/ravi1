...
<li class="cate_nl_hover_cond"><a href="{"categories.view?category_id=`$category.category_id`&company_id=`$smarty.request.company_id`"|fn_url}" style="background-image:none;">
...
{if (!$category.category_id|in_array:$config.category_hide_subcategory)}
 {if $category.subcategories}  {*$category.category_id*}         
        <div id="box{$sl_no}" class="box_submenu {if $popular_brands}box_submenu_background{/if}">
