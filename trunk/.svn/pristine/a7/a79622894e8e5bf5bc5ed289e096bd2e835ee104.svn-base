{* $Id: object_group.tpl 12108 2011-03-25 12:03:53Z subkey $ *}
...
{if !$no_table}
	...
	<div class="object-name">
		...
		{/if}
		<div class="object-group-link-wrap"><a class="cm-external-click{if $non_editable} no-underline{/if}{if $main_link} link{/if}"{if !$non_editable && !$no_rev} rev="opener_group{$id_prefix}{$id}"{/if}{if $main_link} href="{$main_link|fn_url}"{/if}>{$text}</a></div><span class="object-group-details">{$details}</span>
	</div>
</div>
{/if}
