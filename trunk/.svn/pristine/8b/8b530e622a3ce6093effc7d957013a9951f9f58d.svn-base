{* $Id: categories_tree_simple.tpl 12869 2011-07-05 08:26:31Z bimib $ *}
{* --------- CATEGORY TREE --------------*}
		...
		<tr>
			<td class="nowrap" style="padding-left: {$_shift}px;">
				{if $cur_cat.has_children || $cur_cat.subcategories}
					{if $show_all}
					<img src="{$images_dir}/plus.gif" width="14" height="9" border="0" alt="{$lang.expand_sublist_of_items}" title="{$lang.expand_sublist_of_items}" id="on_cat_{$cur_cat.category_id}_{$random}" class="hand cm-combination-cat {if isset($path.$cat_id) || $expand_all}hidden{/if}" />
					{else}
					{if $except_id}
						{assign var="_except_id" value="&except_id=`$except_id`"}
					{/if}
					<img src="{$images_dir}/plus.gif" width="14" height="9" border="0" alt="{$lang.expand_sublist_of_items}" title="{$lang.expand_sublist_of_items}" id="on_cat_{$cur_cat.category_id}_{$random}" class="hand cm-combination-cat {if (isset($path.$cat_id))}hidden{/if}" onclick="if (!$('#cat_{$cur_cat.category_id}_{$random}').children().length) jQuery.ajaxRequest('{"categories.picker?category_id=`$cur_cat.category_id`&random=`$random`&display=`$display`&checkbox_name=`$checkbox_name``$_except_id`"|fn_url:'A':'rel':'&'}', {$ldelim}result_ids: 'cat_{$cur_cat.category_id}_{$random}'{$rdelim})" />
					{/if}
					<img src="{$images_dir}/minus.gif" width="14" height="9" border="0" alt="{$lang.collapse_sublist_of_items}" title="{$lang.collapse_sublist_of_items}" id="off_cat_{$cur_cat.category_id}_{$random}" class="hand cm-combination-cat {if !isset($path.$cat_id) && (!$expand_all || !$show_all)}hidden{/if}" />
				{else}
					&nbsp;
				{/if}</td>
			<td width="100%">
