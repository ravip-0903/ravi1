{* $Id: image.tpl 12666 2011-06-10 11:09:12Z alexions $ *}
...
{if !$image.is_flash}
	{*{if $image.image_x || $href}<a href="{$href|default:$image.image_path|default:$config.no_image_path}" {if !$href}target="_blank"{/if}>*}
	{if $image.image_x || $href}<a href="{$config.ext_images_host}{$image.image_path|default:$config.no_image_path}" {if !$href}target="_blank"{/if}>
{/if}

	<img {if $image_id}id="image_{$object_type}_{$image_id}"{/if} src="{$config.ext_images_host}{$image.image_path|default:$config.no_image_path|fn_generate_thumbnail:$image_width:$image_height}" {if $image_width}width="{$image_width}"{/if} {if $image_height}height="{$image_height}"{/if} alt="{$image.alt}" border="0" {if $close_on_click == true}onclick="window.close();"{/if} />
