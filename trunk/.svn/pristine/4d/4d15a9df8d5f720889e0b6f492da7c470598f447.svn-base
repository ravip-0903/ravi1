	...
	<tr>
		<td style="background-color: #ffffff; border: 1px solid #e6e6e6; margin: 0px auto 0px auto; padding: 0px 44px 0px 46px; text-align: left;">
			...
			<tr>
                ...
                <img src="{if $manifest.Mail_logo.vendor}{$config.images_path}{else}{$images_dir}/{/if}{if $manifest.Mail_logo.vendor}{$manifest.Mail_logo.filename}{/if}" width="{$manifest.Mail_logo.width}" height="{$manifest.Mail_logo.height}" border="0" alt="{$manifest.Mail_logo.alt}" />
                {else}
                	<h2>{$company_name}
                {/if}
                </td>
