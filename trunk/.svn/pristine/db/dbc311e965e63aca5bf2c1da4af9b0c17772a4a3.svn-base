...
// $Id: companies.php 12865 2011-07-05 06:57:22Z 2tl $
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (!empty($_REQUEST['company_data']['company'])) {  // Checking for required fields for new company
			...
			if ((PRODUCT_TYPE == 'MULTIVENDOR' || PRODUCT_TYPE == 'MULTISHOP') && isset($_REQUEST['company_data']['is_create_vendor_admin']) && $_REQUEST['company_data']['is_create_vendor_admin'] == 'Y') {
				...
				if (!empty($_REQUEST['company_data']['admin_username']) && db_get_field("SELECT COUNT(*) FROM ?:users WHERE user_login = ?s", $_REQUEST['company_data']['admin_username']) > 0) {
					...
					if (!empty($_REQUEST['warehouse_data']['address1'])) {

						$insert = fn_insert_warehouse_data($_REQUEST['warehouse_data'],$company_id,$_REQUEST['company_data']['company']);
	...
	if ($mode == 'update') {
		...
		$check = fn_check_warehouse_data($_REQUEST['company_id']);
...

		{
			...
			$update = fn_update_warehouse_data($_REQUEST['warehouse_data'],$_REQUEST['company_id'],$_REQUEST['company_data']['company']);

		}else{

			$insert = fn_insert_warehouse_data($_REQUEST['warehouse_data'],$_REQUEST['company_id'],$_REQUEST['company_data']['company']);
...
if ($mode == 'manage') {
	...
	$warehouse_data = !empty($company_id) ? fn_get_warehouse_data($company_id) : array(); // Add by paresh
	...
	if (!empty($_SESSION['saved_post_data']['company_data'])) {
		...
		$view->assign('warehouse_data',$warehouse_data);
	...
	$view->assign('fulfillment', fn_get_all_fulfillment()); // Add by paresh

	$view->assign('region_list', fn_get_all_region()); // Add by paresh
	...
	if (!defined('COMPANY_ID')) {
		...
		$tabs['warehouse_info'] = array (

			'title' => fn_get_lang_var('warehouse_info'),

			'js' => true

		);

		/* End by Paresh on 10-02-2012 */
