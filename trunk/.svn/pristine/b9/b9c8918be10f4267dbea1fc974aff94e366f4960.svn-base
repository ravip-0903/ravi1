...
//
// $Id: fn.users.php 12865 2011-07-05 06:57:22Z 2tl $
//
...
function fn_get_user_info($user_id, $get_profile = true, &$profile_id = NULL)
{
	...
	if ($get_profile == true) {
		...
		if (empty($profile_data)) {
			$profile_data = db_get_row("SELECT * FROM ?:user_profiles USE INDEX(usr_pro_idx) WHERE user_id = ?i AND profile_type = 'P'", $user_data['user_id']);
...
// get main profile billing data
function fn_get_main_profile_billing_data($user_id){
    return $profile_billing_data = db_get_row("SELECT b_address, b_address_2, b_city,  b_state ,b_country ,b_zipcode ,b_phone FROM ?:user_profiles USE INDEX(usr_pro_idx) WHERE user_id = ?i AND profile_type = 'P'", $user_id);
}

function fn_fill_billing_address($user_data, $billing_data){
...
//
// Get all user profiles with detail
//
function fn_get_user_profiles_data($user_id)
{
	$profile_data = array();
	if (!empty($user_id)) {
		$profile_data = db_get_array("SELECT * FROM ?:user_profiles WHERE user_id = ?i", $user_id);
	}
...
// Get all user profiles with detail
//
function fn_get_user_profile_data($profile_id)
{
	if (!empty($profile_id)) {
		$profile_data = db_get_row("SELECT * FROM ?:user_profiles WHERE profile_id = ?i", $profile_id);
	}
...
//
// Get all user profiles for address book
//
function fn_get_user_profiles_for_address_book($user_id)
{
	$profiles = array();
	if (!empty($user_id)) {
		$profiles = db_get_array("SELECT profile_id, profile_type, profile_name,s_title,s_firstname,s_lastname,s_address, s_address_2, s_city,  s_state ,s_country ,s_zipcode ,s_phone ,verified FROM ?:user_profiles WHERE user_id = ?i", $user_id);
	}
...
function fn_fill_address(&$user_data, &$profile_fields, $use_default = false, $mode = '')
{
       if($mode == 'update_addressbook'){
            $from = 'S';
            $to = 'B';
       }elseif (Registry::get('settings.General.address_position') == 'billing_first' || $use_default) {
		/*$from = 'B';
		$to = 'S';*/
		$from = 'S';
		$to = 'B';
 ...
 */
function fn_update_user($user_id, $user_data, &$auth, $ship_to_another, $notify_user, $send_password = false,$mode = '',$called_from='')
{
	...
	if (fn_user_need_login($user_data['user_type']) && $mode != 'update_addressbook') {
	...
	if (!empty($user_id)) {
		...
		//Code added by rahul on january 2014 to add three extra fields from facebook starts here.
		...
		if(isset($user_data['city']) && $user_data['city'])
		{
			...
			$user_data['city'] = addslashes($user_data['city']);
		}
		if(isset($user_data['state']) && $user_data['state'])
		{
			$user_data['location'] = TRUE;
			$user_data['state'] = addslashes($user_data['state']);
		}

		if($user_data['location'] ==1)
			$user_data['location'] = $user_data['city'].",".$user_data['state'];

		if(isset($user_data['gender']) && $user_data['gender'])
			$user_data['gender'] = strtoupper(substr($user_data['gender'],0,1));
		if(isset($user_data['phone']) && is_numeric($user_data['phone']) && strlen($user_data['phone'])==10)
		...
		//Code added by rahul on january 2014 to add three extra fields from facebook ends here.
        ...
        if(fn_get_lang_var('assign_autogenerated_coupon') == 'Y' && fn_get_lang_var('assign_autogenerated_coupon_count') > '0' && fn_get_lang_var('assign_autogenerated_coupon_promotion_id') != ''){
				$no_of_coupon = fn_get_lang_var('assign_autogenerated_coupon_count');
				$auto_coupon_promotion_id = fn_get_lang_var('assign_autogenerated_coupon_promotion_id');
				$promotion_duration = fn_get_lang_var('assign_autogenerated_coupon_duration');				
				assign_auto_generated_coupon($user_id, $no_of_coupon, $promotion_duration, $auto_coupon_promotion_id);
				Registry::get('view_mail')->assign('has_coupon', 'true');
		}		
	...
	if (empty($ship_to_another)) {
        ...
        // Fill billing info with shipping if needed
	if ($mode == 'update_addressbook') {
		$billing_data = fn_get_main_profile_billing_data($user_id);
                if(count($billing_data)==0){
                    $profile_fields = fn_get_profile_fields($user_data['user_type']);
                    fn_fill_address($user_data, $profile_fields, false,$mode);
                    $profile_type = 'P';
                }else{
                    $user_data = fn_fill_billing_address($user_data,$billing_data);
                    $profile_type = 'S';
                }
			if(isset($user_data['makeprimary'])){
				db_query("UPDATE ?:user_profiles SET profile_type = ?s WHERE user_id = ?i AND profile_type = ?s", 'S', $user_data['user_id'], 'P');
				$user_data['profile_type'] = $user_data['makeprimary'];
			}	
	...
	if (!empty($notify_user)) {
		...
		if ($action == 'add') {
            ...
            fn_instant_mail($user_data['email'], Registry::get('settings.Company.company_users_department'), 'profiles/create_profile_subj.tpl', 'profiles/create_profile.tpl');
		} else {
	...
	} else if ($mode == 'update_addressbook'){
		fn_set_notification('N', fn_get_lang_var('information'), fn_get_lang_var('text_address_is_updated'));
	}
        else{
                fn_set_notification('N', fn_get_lang_var('information'), fn_get_lang_var('text_profile_is_updated'));
        }
	fn_set_hook('register_clues_bucks_hook',$action, $user_data, $current_user_data);
...
function fn_set_scun_cookie($for_all_dir = 'N'){
    if (!empty($_SESSION['auth']['user_id']) && $_SESSION['auth']['user_id'] !=0) {
	$user_id = $_SESSION['auth']['user_id'];	
        $user_data = db_get_row("SELECT firstname, email FROM ?:users WHERE user_id = ?i", $user_id);
        if (!empty($user_data)) {
                $domain = $_SERVER['HTTP_HOST'] == 'localhost' ? '' : '.shopclues.com';
                $user_name = ($user_data['firstname'] != '' ? $user_data['firstname'] : $user_data['email']);
		if($for_all_dir == 'N'){
                    setcookie('scun',base64_encode(json_encode($user_name)),time()+60*60*24*365,'',$domain);
                }elseif($for_all_dir == 'Y'){
                    setcookie('scun',base64_encode(json_encode($user_name)),time()+60*60*24*365,'/',$domain);
                }
        }
    }
