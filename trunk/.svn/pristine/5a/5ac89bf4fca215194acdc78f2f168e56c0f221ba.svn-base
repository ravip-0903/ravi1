{* $Id: manage.tpl 12780 2011-06-24 14:43:27Z alexions $ *}
...
<tr>
	...
	<th width="60%"><a class="{$ajax_class}{if $search.sort_by == "product"} sort-link-{$search.sort_order}{/if}" href="{"`$c_url`&amp;sort_by=product&amp;sort_order=`$search.sort_order`"|fn_url}" rev={$rev}>{$lang.name}</a></th>
    ...
    <th width="15%"><a class="{$ajax_class} sort-link-{$search.sort_order}" href="{"`$c_url`&amp;sort_by=last_update&amp;sort_order=`$search.sort_order`"|fn_url}" rev={$rev}>Last Update</a></th>
	<th>&nbsp;</th>
</tr>
...
<tr {cycle values="class=\"table-row\", "}>
    ...
    <td>
		{if $product.last_update != "0000-00-00 00:00:00"}
			{$product.last_update}
		{else}
			----
		{/if}
