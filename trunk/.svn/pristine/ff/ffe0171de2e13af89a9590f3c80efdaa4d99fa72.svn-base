...
<div class="form-field cm-row-item">
	...
	<input type="hidden" name="{$prefix}[value]" value="Y" />
        {if $bonus_data.bonus == "give_points_pct"}
            {$lang.return_emi_fee_as_cashback}
        {else}
            {$lang.yes}
        {/if}
	...
	{if $schema.bonuses[$bonus_data.bonus].discount_bonuses}
        ...
        <div style="float:left; margin-top:10px;">
        	<label>{$lang.max_discount_value_lbl}</label>
        	<input class="input-text" type="text" name="{$prefix}[max_discount_value]" value="{$bonus_data.max_discount_value}"/>
            {$lang.max_discount_value}
        </div>
	{/if}
</div>
