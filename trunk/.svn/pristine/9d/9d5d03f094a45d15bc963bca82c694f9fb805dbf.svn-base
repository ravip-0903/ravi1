{* $Id: details.tpl 12089 2011-03-22 14:34:09Z alexions $ *}
...
<div id="content_general">
	...
	<div class="item-summary clear center">
		...
		{$lang.by}&nbsp;<span>{$shipment.shipping}</span>{if $shipment.tracking_number}&nbsp;({$shipment.tracking_number}){/if}{if $shipment.carrier}&nbsp;({$smarty.capture.carrier_name|trim}){/if}<br />
        
        {assign var="shipment_details" value=$shipment.shipment_id|get_shipment_data:'row'}
        <span><strong>{$lang.dimention}:</strong> {$shipment_details.height|default:'N/A'}, <strong>{$lang.weight}:</strong> {$shipment_details.weight|default:'N/A'}, <strong>{$lang.package_type}:</strong> {$shipment_details.package_type_name|default:'N/A'}</span>
		</div>
