<?php
    ...
    require  dirname(__FILE__) . '/../../addons/my_changes/func.php';
    ...
    class checkout extends REST {
        ...
        public function Step1(){
            ...
            require  dirname(__FILE__) . '/../../addons/sdeep/func.php';
        ...
        public function set_paymentMethod(&$cart,$method){
            
            if($method != 'cod'){
                $msg = array('status' => "Failed", "msg" => fn_get_lang_var('api_payment_method_not_available'));
                $this->response($this->json($msg), 200);
            }
        ...
        public function Step3(){
            ...
            $this->set_paymentMethod($user_cart, 'cod');            
            ...
            if($max_cod_amount > 0){
                if($response['cart']['total'] > $max_cod_amount ){
                    $response['error']['cod_amount'] = fn_get_lang_var('api_param_max_cod_error');
                }
            }

            if($this->check_cod($response['cart']['products']) == 'NO'){
                $response['error']['cod'] = fn_get_lang_var('api_param_cod_not_allowed');
            }          
            $log['profile_id'] = $this->_request['profile_id'];
            $log['user_id'] = $this->_request['user_id'];
            $this->RecordLogs($log);
        ...
        public function Step4($place_order=0){
            ...
            $max_cod_amount = (Registry::get('config.max_cod_amount')) ? Registry::get('config.max_cod_amount') : '0';
            if($max_cod_amount > 0){
                if($user_cart['total'] > $max_cod_amount){
                    $error = array('status' => "Failed", "msg" =>fn_get_lang_var('api_param_max_cod_error'));
                    $this->response($this->json($error), 200);
                }
            }
            ...
            if($this->check_cod($user_cart['products']) == 'NO'){
                $error = array('status' => "Failed", "msg" =>fn_get_lang_var('api_param_cod_not_allowed'));
                $this->response($this->json($error), 200);
            }
        ...
        public function place_order(){
            ...
            //echo $order_id;die;
            fn_order_placement_routines($order_id, array(), true, '',$api=1);
            if($order_id){
                //log the orders that are being placed through mobiles
                $this->log_orders($order_id,$cart);
        ...
        public function log_orders($order_id,$cart){
            $_data = array (
                        'order_id' => $order_id,

                        'type' => 'Z', //mobile

                        'data' => serialize($cart['user_data'])
                    );
