{* $Id: product_data.tpl 12842 2011-07-04 09:04:13Z bimib $ *}
...
{hook name="products:buttons_block"}
		...
		{elseif (($product.out_of_stock_actions == "S") && ($product.tracking != "O"))}
			...
			<div class="form-field" id="product_notify_email" style="padding: 0px;{if $product_notification_enabled != "Y"} display: none;{/if}">
				<form action="index.php" method="post" enctype="multipart/form-data" class="cm-disable-empty-files cm-ajax">
					...
					<input type="image" class="go-button" title="{$lang.go}" alt="{$lang.go}" src="{$images_dir}/icons/go.gif" />
	...
	{if $show_list_buttons}
		<{if $separate_buttons}div class="buttons-container"{else}span{/if} id="cart_buttons_block_{$obj_prefix}{$obj_id}" class="add-buttons">
