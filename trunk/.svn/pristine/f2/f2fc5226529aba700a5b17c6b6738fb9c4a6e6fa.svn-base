...
include_once (DIR_ROOT . '/payments/ccavenue_files/libfuncs.php3');
	...
	if(!empty($order_info)){
		...
		$order_data['emi_fee'] = $order_info['emi_fee'];
                $order_data['pgw_promo_code'] = $promo_code;
...
?>
	<form name="frmccev" method="post" action="https://www.ccavenue.com/shopzone/cc_details.jsp">
        ...
        <?php if(isset($promo_code) && $promo_code != '') { ?>
...
<?			
		...
		if (!empty($order_info) )
		{ 
			if (fn_check_payment_script('ccavenue_script.php', $Order_Id)) 
			{
