...
//
// $Id: rma.php 12865 2011-07-05 06:57:22Z 2tl $
//
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'update_details') {
		...
		$user = $_SESSION['auth'];
		$time = time();
		db_query("INSERT INTO clues_rma_history (user_id, order_id, status_from, status_to, return_id, comment, datetime) VALUES ('".$user['user_id']."', '".$change_return_status['order_id']."',  '".$change_return_status['status_from']."', '".$change_return_status['status_to']."', '".$change_return_status['return_id']."', '".$_REQUEST['staffcomment']."', '".$time."')");
	...
	if ($mode == 'bulk_export' && !empty($_REQUEST['return_ids'])) {
		if (!empty($_REQUEST['return_ids'])) {
			...
			$_SESSION['export_orders_list']['orders']['data'] = array('order_id' => $_REQUEST['return_ids']);

			unset($_REQUEST['redirect_url']);
			return array(CONTROLLER_STATUS_REDIRECT, "export_orders.export_rma_list?pattern_id=" . $_SESSION['export_orders_list']['orders']['pattern_id']);
		}
