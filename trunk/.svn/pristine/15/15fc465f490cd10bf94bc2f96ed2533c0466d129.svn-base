...
if ($mode == 'delete_layout') {
	...
	//code by ankur to list manage product tab in first
...
function fn_import($pattern, $import_data, $options)
{
	...
	foreach ($import_data as $k => $v) {
		...
		// Added by Sudhir to generate seo url bigin here dt 25th octo 2012
		if (!array_key_exists('name', $v)) {
			if (array_key_exists('product', $v)) {
				$v['name'] = $v['product'];
			}
		}// Added by Sudhir to generate seo url end here dt 25th octo 2012
        ...
        // Added by Sudhir to make a new record even if product code is null bigin here dt 25th octo 2012
		if(($v['product_code'] == '') OR ($v['merchant_reference_number'] == '')){
			$cmp_id = db_get_fields("SELECT cp.company_id FROM cscart_products cp INNER JOIN cscart_companies cc ON cc.company_id=cp.company_id WHERE cc.company='".$v['company']."'");
			if(($v['merchant_reference_number'] == '') AND ($v['product_code'] == '')){
				$_alt_keys['product_code'] = $cmp_id[0];
			} elseif($v['product_code'] == '') {
				$_alt_keys['product_code'] = $v['merchant_reference_number'].'.'.$cmp_id[0];
			}
		} // Added by Sudhir to make a new record even if product code is null end here dt 25th octo 2012
		...
		if (!(isset($pattern['import_skip_db_processing']) && $pattern['import_skip_db_processing'])) {
            ...
            //code by ankur to set default status D if status is no coming in csv file
			...
			if ($object_exists == true) {
				...
				// Added by Sudhir to make a new record even if product code is null bigin here dt 25th octo 2012
			if(($v['merchant_reference_number'] == '') OR ($v['product_code'] == '')){
				if(($v['merchant_reference_number'] == '') AND ($v['product_code'] == '')){
					$_data['product_code']= $primary_object_id.'.'.$cmp_id[0];
				}elseif($v['product_code'] == '') {
					$_data['product_code']= $v['merchant_reference_number'].'.'.$cmp_id[0];
				}
			} // Added by Sudhir to make a new record even if product code is null end here dt 25th octo 2012
			//added by ankur to add user id who update the product
			  $_data['last_update_by']=$_SESSION['auth']['user_id'];
			//code end
				db_query('UPDATE ?:' . $pattern['table'] . ' SET ?u WHERE ?w', $_data, $primary_object_id);
				//code by arpit
				if($_data['transfer_price'] && $_data['start_date'] && $_data['end_date']){
					$pro_id_temp = $primary_object_id['product_id'];
					db_query("update clues_product_TP set latest='0' where product_id='".$pro_id_temp."' and latest = '1' and type = 'tp'");
					db_query("insert into clues_product_TP set product_id='".$pro_id_temp."',tp='".$_data['transfer_price']."',start_date=".$_data['start_date'].",end_date=".$_data['end_date'].",type='tp',user='".$_SESSION['auth']['user_id']."',date='".date("Y-m-d")."'");
				}
                //code by arpit ends
				
			} else {
			//added by ankur to add user id who insert the product
			  $_data['last_update_by']=$_SESSION['auth']['user_id'];
			//code end
			...
			if(($v['merchant_reference_number'] == '') OR ($v['product_code'] == '')){
				if(($v['merchant_reference_number'] == '') AND ($v['product_code'] == '')){
					db_query('UPDATE cscart_products SET product_code="'.mysql_insert_id().'.'.$cmp_id[0].'" WHERE product_id='.mysql_insert_id());
				}elseif($v['product_code'] == '') {
					db_query('UPDATE cscart_products SET product_code="'.$v['merchant_reference_number'].'.'.$cmp_id[0].'" WHERE product_id='.mysql_insert_id());
				}
			} // Added by Sudhir to make a new record even if product code is null end here dt 25th octo 2012

				//code by arpit
				if($_data['transfer_price'] && $_data['start_date'] && $_data['end_date']){
					$pro_id_temp = mysql_insert_id();
					db_query("update clues_product_TP set latest='0' where product_id='".$pro_id_temp."' and latest = '1' and type = 'tp'");
					db_query("insert into clues_product_TP set product_id='".$pro_id_temp."',tp='".$_data['transfer_price']."',start_date=".$_data['start_date'].",end_date=".$_data['end_date'].",type='tp',user='".$_SESSION['auth']['user_id']."',date='".date("Y-m-d")."'");
				}
                //code by arpit ends
				
				$comp_info=db_get_row("select c.fulfillment_id,c.is_cod from cscart_companies c inner join cscart_products p on p.company_id=c.company_id where p.product_id='".$o_id."'");
				//code by ankur to set default shipping estimation according to merchant of created product
				 
				  if($comp_info['fulfillment_id']==1)
				  $del_time=1;
				  else if($comp_info['fulfillment_id']==2)
				  $del_time=3;
				  else if($comp_info['fulfillment_id']==3)
				  $del_time=4;
				  
				  db_query("update cscart_products set product_shipping_estimation='".$del_time."' where product_id='".$o_id."'");
				//code end
				
				//code by ankur to set cod option according to setting at merchant level
				   if($comp_info['is_cod']==1)
				   $is_cod='Y';
				   else if($comp_info['is_cod']==2)
				   $is_cod='N';
				   
				   db_query("update cscart_products set is_cod='".$is_cod."' where product_id='".$o_id."'");
				//code end
		...
		//code by ankur to place required field validation in the end
		 $pro_arr[$primary_object_id['product_id']]=$primary_object_id['product_id'];
        //code end
		...
		if (!empty($processing_groups)) {
			...
			foreach ($processing_groups as $group) {
                ...
                //code by ankur to show error message
				if(!isset($count[$group['this_field']]))
				$count[$group['this_field']]=0;
				//code end
				...
				if($result==false)
				{
					...
					if($group['this_field']!='detailed_id')
					{
						$count[$group['this_field']]++;
						$csv_header[$group['this_field']]['Warning']=$count[$group['this_field']];
						if ($object_exists == false)
						{
							if($group['this_field']=='company' || $group['this_field']=='Category' )
							{
								fn_delete_product($args[0]);
								$pro_deleted=1;
								$deleted_count++;
								$processed_data['N']--;
								break;
        ...
        //change by ankur to not allow  creation of product reference if product get deleted
		if($pro_deleted==0)
		{
			if (!(isset($pattern['import_skip_db_processing']) && $pattern['import_skip_db_processing'])) {
	...
	//code by ankur for adding required field validation
	$disable_count=0;
	
    foreach($pro_arr as $pro_id=>$value)
	{
		
		$cur_status=db_get_field("select status from cscart_products where product_id='".$pro_id."'");
		
		if(!empty($cur_status) && $cur_status=='A')
		{
			$validation=fn_check_product_required_field($pro_id);
			if(!$validation['validation'])
			{
				db_query("update cscart_products set status='D' where product_id='".$pro_id."'");
				$disable_count++;
			}
	...
	$msg2 = fn_get_lang_var('text_exim_data_warning');
	
	$string='';
	if(!empty($csv_header))
	{
		foreach($csv_header as $key=>$count)
		{
			$string.="<b>".$key."</b> Of <b>".$count['Warning']."</b> Products Does Not Inserted/Updated<br/>";
		}
	}
	if($string!='')
	{
		$msg2 = str_replace('[Warning]',$string,$msg2);
		fn_set_notification('E',fn_get_lang_var('important'), $msg2);
	}
	
	if($disable_count!=0)
	{
		$msg3=fn_get_lang_var('missing_required_field');
		$msg3 = str_replace('[count]',$disable_count,$msg3);
		fn_set_notification('E',fn_get_lang_var('important'), $msg3);
	}
	
	if($deleted_count!=0)
	{
		$msg3=fn_get_lang_var('company_category_missing_deleted');
		$msg3 = str_replace('[count]',$deleted_count,$msg3);
		fn_set_notification('E',fn_get_lang_var('important'), $msg3);
	}
