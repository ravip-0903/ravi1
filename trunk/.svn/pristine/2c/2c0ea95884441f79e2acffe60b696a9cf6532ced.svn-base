...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && $_REQUEST['report_type'] !='') {
	if($mode == 'reports')
	{
		...
		if($time_from != '' && $time_to != '') {
			$query_order_rows_where_segment="AND co.timestamp between '".strtotime($time_from)."' and '".strtotime($time_to)."'";
		} else {
			$query_order_rows_where_segment = '';	
		}
		
		if($report_type == 'full') {
			$order_by = $_REQUEST['order_by'];			
			$query_milkrun_full = "SELECT cc.company, cc.address, cpd.product, cod.amount, 
								cp.merchant_reference_number, cod.product_code, co.order_id, cc.state, cs.carrier 
								...
								LEFT JOIN cscart_shipments cs ON csi.shipment_id = cs.shipment_id WHERE co.status='C' $query_order_rows_where_segment order by cc.$order_by ASC";
		...
		} else if($report_type == 'merchant')
		{
			$query_milkrun_full = "	SELECT *
									...
									LEFT JOIN cscart_companies cc ON co.company_id = cc.company_id
									LEFT JOIN cscart_product_descriptions cpd ON cod.product_id = cpd.product_id
									WHERE co.status='C' $query_order_rows_where_segment";
