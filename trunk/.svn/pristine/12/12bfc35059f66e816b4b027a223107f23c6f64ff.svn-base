...
{if $items.companies}
	...
	{foreach from=$items.companies item=v key=k}
		...
		<li>{include file="addons/sdeep/common_templates/vendor_icons.tpl" vendor_id=$k size="10"}<a href="{"companies.view?company_id=`$k`"|fn_url}">{$v}{if $rating} ({$rating}){/if}</a></li>
		{* [/andyye] *}
	{/foreach}
	</ul>
