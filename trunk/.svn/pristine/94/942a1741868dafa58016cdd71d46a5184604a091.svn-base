...
if ($mode == 'add_product') {
	...
	if (!empty($params['category_id'])) {
		$parent_ids = explode('/', db_get_field("SELECT id_path FROM ?:categories IGNORE INDEX(p_category_id) WHERE category_id = ?i", $params['category_id']));
	...
	if(Registry::get('config.solr')) {
		$view_all_filter = fn_get_solr_products_brand($params);
	} else {
		if(Registry::get('config.category_solr')) {
			$view_all_filter = fn_get_solr_categories($params);
...
//function by ankur to sort brands by index and placed non alphanumeric index in the last
function fn_get_sorted_brands($view_all_filter)
{
	foreach($view_all_filter as $key=>$value)
	{
		$key_test=$key.'A';
		if(ctype_alnum($key_test))
		{
			$new_array[$key]=$value;
		}
		else
