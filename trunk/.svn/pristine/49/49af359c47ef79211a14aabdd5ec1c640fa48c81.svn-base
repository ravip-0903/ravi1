...
<tr>
	...
	<th width="15%">
		<a class="{$ajax_class}" href="" rev="pagination_contents">{$lang.internal_name}</a></th>
	<th width="15%">
		<a class="{$ajax_class}" href="" rev="pagination_contents">{$lang.condition}</a></th>
	<th width="10%">
...
<tr {cycle values="class=\"table-row\", "}>
	...
	<td>
		<input type="text" name="promotions[{$promotion.promotion_id}][name]" size="25" value="{$promotion.name}" class="input-text" /></td>
	<td>
		<input type="text" name="promotions[{$promotion.promotion_id}][internal_name]" size="25" value="{$promotion.internal_name}" class="input-text" /></td>

	{assign var=condition value=";"|explode:$promotion.conditions_hash}
	<td>
		{foreach from=$condition item=cond} {$cond|truncate:40} <br /> {/foreach}
	</td>
	<td>
		<input type="text" name="promotions[{$promotion.promotion_id}][priority]" size="45" value="{$promotion.priority}" class="input-text-short" /></td>
	<td>
