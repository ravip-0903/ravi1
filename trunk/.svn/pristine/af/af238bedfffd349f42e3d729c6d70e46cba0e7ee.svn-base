<h1 class="cat_title" style="margin-top:5px !important;">{$lang.most_viewed_product_heading_nl}</h1>
<!--Hot Deals -->
...
{foreach from=$products item="sproducts" key=k}
    ...
    {assign var="deal_title" value=$k|get_deal_title:'MOSTVIEWED'}
    {if $sproducts|count}
    <!-- Heading -->
    <h1 class="block-packs-title">
    <span class="float_left">
          {if $deal_title != ''}
          {$lang.most_viewed_cat_heading}
          {$deal_title}
          {else}
          {$lang.most_viewed_deal_title_not_found}
          {/if}
        </span>
    </h1>
<!-- End Heading -->
