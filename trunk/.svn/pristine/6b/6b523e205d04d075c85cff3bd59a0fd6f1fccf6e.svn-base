...
if($mode == 'list')
{
	$categories = db_get_array("select * from clues_merchandise_data where type='FEATURED' order by position asc");
	...
	if(!empty($categories) && count($categories)>0) {
		...
		foreach($categories as $category) {
			...
			$params['limit'] = 20;
			$products[$category['id']] = fn_featuredproduct_get_products($params);
			//echo '<pre>';print_r($params);
			
		}
		fn_add_breadcrumb(fn_get_lang_var('featured_product'));
