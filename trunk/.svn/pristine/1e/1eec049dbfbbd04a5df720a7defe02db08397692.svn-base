...
{** block-description:clues_wrapper_3x1 **}
<div class="grid_3">
...
<ul id="{$block.block_id}" class="jcarousel-skin-ie7">
    ...
    {foreach from=$items item="product" name="productlisting"}
...
width:72px; bottom:-2px ; left:0;"><a class="nl_prc_red_icon_spl" href="{"products.view?product_id=`$product.product_id`"|fn_url}" onClick="_gaq.push(['_trackEvent', 'Homepage', 'Click', '{$block.description|replace:" ":"_"}_{$smarty.foreach.topproductlisting.iteration}']);">{$lang.see_special_offer_inside}</a></span> 
  ...
  //<![CDATA[  
        $("#{$block.block_id}").jcarousel({$ldelim}
            size: {$items|count},
		    scroll:1,
		    item_count: {$items|sizeof}
        {$rdelim});  
  //]]>
  		var h = $('#cus_block_{$block.block_id}').val();
		if(h == '210'){$ldelim}
			$('ul#{$block.block_id} > li').css('height','305px');
			$('.cus_{$block.block_id}').css('height','305px');
			$('.cus_{$block.block_id} .jcarousel-skin-ie7 .jcarousel-container .jcarousel-prev-horizontal').css('top','170px');
			$('.cus_{$block.block_id} .jcarousel-skin-ie7 .jcarousel-container .jcarousel-next-horizontal').css('top','170px');
		{$rdelim}
        </script>
</div>
