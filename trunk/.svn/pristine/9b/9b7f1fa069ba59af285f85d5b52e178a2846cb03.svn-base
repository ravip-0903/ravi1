...
$schema = array (
	...
	'export_fields' => array (
		...
		'Merchant Ref' => array (
			'db_field' => 'merchant_reference_number'
		),
		'MFG ref' => array (
			'db_field' => 'manufacturer_reference_number'
		),
		'MFG ref Type' => array (
			'db_field' => 'manufacturer_reference_type'
		),
		'Shipping Estimate' => array (
			'db_field' => 'product_shipping_estimation'
		),
		'Status' => array (
			'db_field' => 'status'
		),
...
function fn_exim_get_taxes($product_taxes, $lang_code = '')
{
	...
	$taxes = db_get_fields("SELECT tax FROM ?:tax_descriptions WHERE ?s LIKE tax_id  AND lang_code = ?s", $product_taxes, $lang_code);
...
function fn_exim_get_localizations($localization_ids, $lang_code = '')
{
	...
	$locs = db_get_fields("SELECT localization FROM ?:localization_descriptions WHERE ?s LIKE localization_id AND lang_code = ?s", $localization_ids, $lang_code);
...
function fn_exim_send_product_notifications($primary_object_ids, $import_data)
{
	...
	foreach ($primary_object_ids as $k => $v) {
		$product_data = fn_get_product_data($v['product_id'], $auth, DESCR_SL, '', true, true, true, true);
		if ($product_data['amount'] > 0) {
			fn_send_product_notifications($v['product_id'], $product_data['product']);
		}
	}
