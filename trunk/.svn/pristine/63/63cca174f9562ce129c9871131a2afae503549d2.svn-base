jQuery.extend({
	...
	processForms: function(elm)
	{
		...
		if (jQuery.area == 'A') {
			...
			$('textarea.cm-wysiwy', elms).appear(function() {
                               $(this).ceEditor();
			});
		...
		for (i=0; i < elms.length; i++) {
			...
			if (elm_id) {
				...
				if (set_mark == true) {
					...
					//code by ankur to hide please wait msg if validatoin failed
				
					if($('#product_page').val()=='true')
					{
						
						$('.buttons').show();
						$('#add_update_label').hide();
					}
					//code end
		...
		_init: function(params) {
			...
			container.dialog({
				...
				open: function(e, u) {
					...
					$('textarea.cm-wysiwy', d).ceEditor('recover');
				},

				beforeClose: function(e, u) {
					var d = $(this);
					$('textarea.cm-wysiwy', d).ceEditor('destroy');
				},
