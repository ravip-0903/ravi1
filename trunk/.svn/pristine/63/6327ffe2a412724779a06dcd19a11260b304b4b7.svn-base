...
$wishlist = & $_SESSION['wishlist'];
	/*added by clues dev to fetch the wishlist from db when not found in session*/
if((isset($wishlist['products']) && (count($wishlist['products']) == '0'))){
    ...
    fn_extract_wishlist_content_with_session_id($_SESSION['wishlist'], $session_id, 'W', $user_type);
	$wishlist = & $_SESSION['wishlist'];
}
	/*added by clues dev to fetch the wishlist from db when not found in session*/
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'add') {
		...
		/* Constructing array for request coming from ajaxified pages */
		if($_REQUEST['wishlist_ajaxified_pages']==1)
		{
			$_REQUEST['product_data'][$_REQUEST[product_id]]=array('product_id'=>$_REQUEST[product_id]);
		}
		/* Ends Here */
        ...
        $view->assign('productId', $product_id);
		...
		if (defined('AJAX_REQUEST') || ($_REQUEST['wishlist_ajaxified_pages']==1)) {
			...
			/* Constructing response for ajaxified pages */
			if($_REQUEST['wishlist_ajaxified_pages']==1)
			{
				$data = array('title'=>$title,'msg'=>$msg);
