...
<div id="content_log" class="hidden">
	...
	<tr>
        ...
        <th width="30%"><a class="{$ajax_class}" href="{"`$c_url`&amp;sort_by=order_id&amp;sort_order=`$sort_order`"|fn_url}" rev="pagination_contents">{$lang.order_id}</a>{if $sort_by == "order_id"}{$sort_sign}{/if}</th>
	</tr>
	...
	<tr {cycle values=",class=\"table-row\""}>
		...
		<ul>
           ...
           {else}
                ...
                {assign var="prev_bal" value=$l.debit}
            {/if}
		</ul>
		</td>
		<td>
        ...
        <td>
          {if $l.order_id}<a href="{"orders.details?order_id=`$l.order_id`&amp;selected_section=payment_information"|fn_url}" class="underlined">&nbsp;{$l.order_id}&nbsp;</a>{else}-{/if}
        </td>
		</tr>
		{foreachelse}
		<tr>
