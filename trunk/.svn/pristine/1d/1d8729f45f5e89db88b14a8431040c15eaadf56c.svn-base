...
//
// $Id: shipments.php 12865 2011-07-05 06:57:22Z 2tl $
//
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'add' && !empty($_REQUEST['shipment_data'])) {
		if (!empty($_REQUEST['shipment_data']['products']) && fn_check_shipped_products($_REQUEST['shipment_data']['products'])) {
			...
			$shipment_data['tracking_number'] = trim($shipment_data['tracking_number']);
			$shipment_id = db_query("INSERT INTO ?:shipments ?e", $shipment_data);

			foreach ($_REQUEST['shipment_data']['products'] as $key => $amount) {
				
				// if condition is introducted by HPRAHI to recreate a shipment
				if(!isset($_REQUEST['reshipment']))
				{
				...
				{
					$order_info['items'][$key]['amount'] = intval($amount);
				}
				$_data = array(
					'item_id' => $key,
