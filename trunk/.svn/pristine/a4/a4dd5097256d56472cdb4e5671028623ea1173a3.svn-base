...
<div id="page_2" class="whole_sale_moq {if $new_width_val=="33"}grid_3{elseif $new_width_val=="50"}grid_2 {else}grid_3{/if}">
...
{/if}
   ...
   {if $product.is_wholesale_product == 0}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="box_metacategory_name" alt="{$product.product}" title="{$product.product}">{/if}
    <div style="text-align:center;">{$product.product|truncate:40:"...."}</div>
...
<span class="whole_sale_min_qty">{$lang.min_quantity}:{$product.min_qty}</span>
{/if}
...
{if $after_apply_promotion !=0}
    ...
    {if $product.price_see_inside ==0}
        <span class="box_metacategory_priceoffer_mobile" style="color: #900; position: absolute; left:0; bottom:18px; font:bold 16px trebuchet ms; margin-top: -3px; float: left;">{if $product.is_wholesale_product == 1}{$lang.price_per_unit}{/if}{$after_apply_promotion|format_price:$currencies.$secondary_currency:""}</span>
    ...
    {/if}
{else}
    <span class="box_metacategory_priceoffer">{if $product.is_wholesale_product == 1}{$lang.price_per_unit}{/if}{$product.price|format_price:$currencies.$secondary_currency:""}</span>
