<?php
	...
	class ivr extends REST {
			...
			parent::__construct();				// Init parent contructor
               ...
               public function ivr(){
                   ...
                   if($this->get_request_method() == "GET"){
                        ...
                        if(empty($params['order_id']) && empty($params['phone'])){
                            $msg = array('status' => "Error", "msg" => "Invalid Parameters");
                            $this->response($this->json($msg), 400);  
                        }

                        if(!(is_numeric($params['order_id']) || empty($params['order_id']))){
                            $msg = array('status' => "Error", "msg" => "Invalid Parameters");
                            $this->response($this->json($msg), 400);
                        }elseif(!(is_numeric($params['phone']) || empty($params['phone']))){
                            $msg = array('status' => "Error", "msg" => "Invalid Parameters");
                            $this->response($this->json($msg), 400);
                        }

			if(!empty($params['phone'])){
				//$last_order = db_get_array("SELECT order_id, status FROM cscart_orders WHERE b_phone='".$params['phone']."' AND status NOT IN ('F', 'N') ");
				$last_order = db_get_row("select concat(o.firstname, ' ', o.lastname ) as full_name, s_city as city,
		cd1.category_id as meta_category_id,
		IF(cd1.category_id is null, 'Gift Certificates' , cd1.category) as meta_category_name,
		cd2.category_id as category_id,
		cd2.category as category_name,
		o.order_id,
		o.status
		FROM cscart_orders o
		LEFT JOIN cscart_order_details od ON od.order_id = o.order_id
		LEFT JOIN cscart_products_categories pc ON pc.product_id = od.product_id and pc.link_type='M'
		...
		LEFT JOIN cscart_category_descriptions cd2 ON cd2.category_id = SUBSTRING_INDEX(SUBSTRING_INDEX(id_path, '/',2), '/',-1)
		where o.status NOT IN ('F','N','T', 'D') AND o.b_phone='".$params['phone']."' order by o.order_id desc limit 1");

				if(empty($last_order)){
					$last_order = array("status"=>"no_valid_order_found", "error"=>"No Valid Order found for this phone");
				}

				$this->response($this->json($last_order), 200);
			}
		...
		function getOrderStatus($order_id, $notify = false){
		   ...
		   // end by ajay
			...
			if($ord_status['status'] == 'A' || $ord_status['status'] == 'C'){ // If Order is in Shipped State
				...
				if($notify){
					if($ord_status['status'] == 'A'){ // If Order is in Shipped State
					        require_once  dirname(__FILE__) . '/../../addons/my_changes/func.php';
