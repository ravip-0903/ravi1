<?php
	...
	if(!empty($order_info)){
		...
		$custPhoneNo1 = '';
		$custPhoneNo2 = '';
		$custPhoneNo3 = $order_info['b_phone'];
		$custMobileNo = '';
		...
		$deliveryPhNo1 = '';
		$deliveryPhNo2 = '';
		$deliveryPhNo3 = $order_info['s_phone'];
		$deliveryMobileNo = '';
		$otherNotes = 'Test order';
		$editAllowed = 'N';
		$requestparameter = $config['direcpay_merchantid'].'|DOM|IND|INR|'.$order_info['display_subtotal'].'|'.$order_info['order_id'].'|others|'.$config['direcpay_responseurl'].'|'.$config['direcpay_responseurl'].'|'.$config['collaborator_id'];
?>
