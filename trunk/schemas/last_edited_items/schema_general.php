...
if (isset($_SESSION[$prefix . '_' . $description])) {
	$data = base64_decode($_SESSION[$prefix . '_' . $description]);
	list($user_id, $_data) = explode(':', $data);
	$data = $_data;
} else {
	$data = '';
}
...
if (empty($data) || $_data == base64_decode('TElDRU5TRV9JU19JTlZBTElE')) {
	$time = intval(fn_get_setting_description('70024'));
	
	$schema = array(
		'edit' . '_action' => (empty($time) || $time < (TIME - SECONDS_IN_DAY * 6 * 5)) ? true : false,
...
} elseif ($_data != base64_decode('QUNUSVZF')) {
