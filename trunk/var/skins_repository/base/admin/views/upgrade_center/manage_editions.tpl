{* $Id: manage_editions.tpl 12671 2011-06-14 06:08:41Z subkey $ *}
			...
			{if $package.is_avail == 'Y'}
				<form action="{""|fn_url}" method="get" name="uc_form_{$package.name|md5}" >
					...
					{else}
						...
						<p>{$lang.text_uc_buy_new_edition_license|replace:"[url]":$uc_settings.updates_server}</p>
					{/if}
				</form>
			{elseif $package.purchase_time_limit == 'Y'}
				<span>{$lang.upgrade_is_not_avail|replace:"[url]":$uc_settings.updates_server}</span>
			{else}
				<span>{$lang.update_period_expired|replace:"[url]":$uc_settings.updates_server}</span>
			{/if}
		{/if}
	</div>
