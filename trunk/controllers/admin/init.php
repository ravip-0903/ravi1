...
function fn_check_priviledges($privi_name){
	$query = "SELECT cu.user_id FROM `cscart_users` cu "
		  ...
		  . "' AND (cu.user_login = 'admin' OR cup.privilege = '".$privi_name."')";
	$manage_privi_links = db_get_field($query);
	return $manage_privi_links;
}

$manage_milkruns_links = fn_check_priviledges("manage_milkruns");
$view->assign('manage_milkruns_links', $manage_milkruns_links);

$manage_shipment_links = fn_check_priviledges("manage_shipment");
$view->assign('manage_shipment_links', $manage_shipment_links);

$manage_manifest_links = fn_check_priviledges("manage_manifest");
$view->assign('manage_manifest_links', $manage_manifest_links);

$manage_bulk_shipments_links = fn_check_priviledges("manage_bulk_shipments");
$view->assign('manage_bulk_shipments_links', $manage_bulk_shipments_links);

/*$query = "SELECT cu.user_id FROM `cscart_users` cu "
		  . " LEFT JOIN `cscart_usergroup_links` cul ON cu.user_id = cul.user_id"
		  . " LEFT JOIN cscart_usergroup_privileges cup ON cul.usergroup_id = cup.usergroup_id "
		  . " WHERE cu.status = 'A' AND cu.user_id = '" . $_SESSION['auth']['user_id'] 
		  . "' AND (cu.user_login = 'admin' OR cup.privilege = 'manage_shipment')";
$manage_shipment_links = db_get_field($query);
$view->assign('manage_shipment_links', $manage_shipment_links);*/
