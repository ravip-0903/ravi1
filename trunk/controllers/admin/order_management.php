...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'update') {
		...
		if ($action == 'continue') {
			...
			foreach($_REQUEST['cart_products'] as $item)
			{
				$compid[]=db_get_field("select company_id from cscart_products where product_id='".$item['product_id']."'");
			}
			if(count(array_unique($compid))==1)
			{
				$_suffix = '.customer_info';
			}
			else
			{
				$msg=fn_get_lang_var('more_than_one_merchants_product');
...
//this is for checking the cart product whether they are of different merchant or not
