...
//
// $Id: index.php 12865 2011-07-05 06:57:22Z 2tl $
//
...
if ($mode == 'index') {
	...
	{
		//$condition = " AND status IN ('A','B','C','E','G','H','J','K','L','M','O','P')";
		$vendor_status = Registry::get('config.vendor_status');
		$status_cond = "'".implode("','",$vendor_status)."'";
		$condition = " AND status IN (".$status_cond.")";
	}
	/*End by paresh*/
	...
	if (!empty($latest_orders)) {
		$order_items = db_get_array('SELECT ?:order_details.order_id, ?:order_details.amount, ?:product_descriptions.product FROM ?:order_details LEFT JOIN ?:product_descriptions ON ?:order_details.product_id = ?:product_descriptions.product_id AND ?:product_descriptions.lang_code = ?s WHERE ?:order_details.order_id IN (?a) ORDER BY ?:product_descriptions.product', CART_LANGUAGE, array_keys($latest_orders));
	...
	/*added by clues dev*/
	$order_status_exclude_for_accounting = Registry::get('config.order_status_exclude_for_accounting');
	$order_status_exclude_for_accounting_condition = "'".implode("','",$order_status_exclude_for_accounting)."'";
	/*added by clues dev*/
	// Dayly orders
	...
	$orders_stats['daily_orders']['totals'] = db_get_row("SELECT SUM(IF(status NOT IN ($order_status_exclude_for_accounting_condition), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition(), mktime(0, 0, 0, $today['mon'], $today['mday'], $today['year']), TIME);
	$orders_stats['daily_orders']['prev_totals'] = db_get_row("SELECT SUM(IF(status NOT IN ($order_status_exclude_for_accounting_condition), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition(), mktime(0, 0, 0, $last_day['mon'], $last_day['mday'], $last_day['year']), TIME - SECONDS_IN_DAY);
	...
	$orders_stats['weekly_orders']['totals'] = db_get_row("SELECT SUM(IF(status NOT IN ($order_status_exclude_for_accounting_condition), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition(), mktime(0, 0, 0, $wstart['mon'], $wstart['mday'], $wstart['year']), TIME);
	$orders_stats['weekly_orders']['prev_totals'] = db_get_row("SELECT SUM(IF(status NOT IN ($order_status_exclude_for_accounting_condition), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition(), mktime(0, 0, 0, $last_wstart['mon'], $last_wstart['mday'], $last_wstart['year']), mktime(0, 0, 0, $wstart['mon'], $wstart['mday'], $wstart['year']));
	...
	$orders_stats['monthly_orders']['totals'] = db_get_row("SELECT SUM(IF(status NOT IN ($order_status_exclude_for_accounting_condition), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i  AND timestamp <= ?i $condition " . fn_get_company_condition(), mktime(0, 0, 0, $today['mon'], 1, $today['year']), TIME);
	$orders_stats['monthly_orders']['prev_totals'] = db_get_row("SELECT SUM(IF(status NOT IN ($order_status_exclude_for_accounting_condition), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i  AND timestamp <= ?i $condition " . fn_get_company_condition(), mktime(0, 0, 0, $last_month['mon'], 1, $last_month['year']), mktime(0, 0, 0, $last_month['mon'], 1, $last_month['year']) + (TIME - mktime(0, 0, 0, $today['mon'], 1, $today['year'])));
	...
	$orders_stats['year_orders']['totals'] = db_get_row("SELECT SUM(IF(status NOT IN ($order_status_exclude_for_accounting_condition), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition(), mktime(0, 0, 0, 1, 1, $today['year']), TIME);
