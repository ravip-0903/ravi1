...
//
// $Id: discussion.php 12865 2011-07-05 06:57:22Z 2tl $
//
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'add_post') {
		...
		if (!empty($post_data['thread_id'])) {
			...
			//code by ankur to check the restrict ip and phrase
			...
			if(!empty($rest_check))
			{
				...
				fn_set_notification('E', fn_get_lang_var('error'), fn_get_lang_var('ip_blocked'));
			...
			foreach($result as $phrase)
			{
				...
				if(preg_match("/".$phrase['restrict_phrase']."/i", $review))
				{
			...
			if(!empty($rest_ph_match))
			{
				...
				$msg.="<br/><br/>Restrcited Phrase Found-".implode(",",$rest_ph_match);
				sendElasticEmail($to, $sub, " ", $msg, $from, fn_get_lang_var('review_mail_header'), '');
				
				fn_set_notification('E', fn_get_lang_var('error'), fn_get_lang_var('phrase_blocked'));
	...
	if ($mode == 'add_cpost') {
		...
		$post_data = $_REQUEST;
		if($post_data['captcha_number'] != '' && $post_data['verification_code'] != ''){
			if($post_data['captcha_number'] != $post_data['verification_code']){
				echo 'captcha_error';				
				exit;
			}				
		}elseif($post_data['captcha_number'] == '' || $post_data['verification_code'] == ''){
			echo 'captcha_code_missing';
			exit;
		}
                
            // Code by Raj to check the enter character length   
	...
	if ($mode == 'update_posts') {
		...
		if (AREA == 'A' && !empty($_REQUEST['posts']) && is_array($_REQUEST['posts'])) {
			...
			foreach ($_REQUEST['posts'] as $p_id => $data) {
				...
				if(strpos($_REQUEST['redirect_url'], 'dispatch=products')>1)
				 { 
					 db_query("update clues_user_product_rating set for_product_status='".$data['status']."' where id='".$p_id."' ");
				 }
				 else if(strpos($_REQUEST['redirect_url'], 'dispatch=companies')>1)
				 {
					
					 db_query("update clues_user_product_rating set for_merchant_status='".$data['status']."' where id='".$p_id."' ");
				 }
