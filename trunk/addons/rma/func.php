...
function fn_get_rma_properties($type = RMA_REASON, $lang_code = CART_LANGUAGE)
{
	...
	return db_get_hash_array("SELECT a.*, b.property,b.description FROM ?:rma_properties AS a LEFT JOIN ?:rma_property_descriptions AS b ON a.property_id = b.property_id AND b.lang_code = ?s WHERE a.type = ?s $status ORDER BY a.position ASC", 'property_id', $lang_code, $type);
}
...
{
	//change by ankur to fix the return period bug.
	$return_period = fn_is_returnable_product($product_id);
	if ((false !=$return_period) && !empty($cart['products'][$_id]['product_id'])) {
...
function fn_rma_get_order_info(&$order, &$additional_data)
{
	if (!empty($order)) {
        ...
        //change by ankur to get flag on order history page
		  if($status_data['allow_return'] == 'N')
		  {
			 $order['order_return_status'] = 'N'; 
		  }
		  
		//code end
		if (!empty($status_data) && $status_data['allow_return'] == 'Y' && isset($additional_data[ORDER_DATA_PRODUCTS_DELIVERY_DATE])) {
			
			//added by ankur to get wheather return period expired or not
			  $return_expired='Y';
			  foreach($order['items'] as $k=>$v)
			  {
				  if (isset($v['extra']['return_period']) &&  true == fn_check_product_return_period($v['extra']['return_period'], $additional_data[ORDER_DATA_PRODUCTS_DELIVERY_DATE]))
				  {
					  $return_expired='N';
				  }
			  }
			 if($return_expired=='Y' && !empty($order['items']))
			 {
				 //function written by ankur to calculate the days of expired
				$exp_days= fn_rma_get_expired_days($v['extra']['return_period'],$additional_data[ORDER_DATA_PRODUCTS_DELIVERY_DATE]);
				if($exp_days<=7)
				 $order['order_return_status'] = 'E';  //E means expired
				else
				$order['order_return_status'] = 'N';
			 }
			 else if(empty($order['items'])) //this is for GC order
			 {
				 $order['order_return_status'] = 'N';
			 }
			//code end
...
function fn_rma_get_expired_days($return_period,$timestamp) 
{
	$ret_time=$return_period*86400+$timestamp;
	$time_diff=TIME-$ret_time;
