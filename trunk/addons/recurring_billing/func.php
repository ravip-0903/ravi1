...
function fn_get_recurring_subscriptions($params, $additional_data = array(), $items_per_page = 0)
{
	...
	if (!empty($additional_data['user_id'])) {
		...
		$condition .= db_quote(" AND '".implode(',', $additional_data['order_ids'])."' LIKE ?:recurring_subscriptions.order_id AND ?:recurring_subscriptions.status = 'A'");
	}
...
function fn_remove_subscription_usergroup($subscription_ids)
{
	...
	foreach ($plan_ids as $plan_id => $users) {
		...
		$usergroups = db_get_hash_single_array("SELECT usergroup_id, recurring_plans_ids FROM ?:usergroups WHERE recurring_plans_ids LIKE ?i", array('usergroup_id', 'recurring_plans_ids'), "'%".$plan_id."%'");
		if (!empty($usergroups)) {
...
function fn_apply_subscription_usergroup($subscription_id)
{
	...
	$usergroup_ids = db_get_fields("SELECT usergroup_id FROM ?:usergroups WHERE recurring_plans_ids LIKE '%".$subscription_data['plan_id']."%' AND status = 'A'");
...
function fn_get_recurring_events()
{
	...
	if (!empty($addon_settings['rb_attempt_period']) && !empty($addon_settings['rb_fail_attempts'])) {
		...
		$failed_sub = db_get_fields("SELECT subs.subscription_id FROM ?:orders as ord INNER JOIN ?:recurring_subscriptions as subs ON subs.order_ids LIKE ord.order_id WHERE subs.status = 'A' AND ord.timestamp > ?i AND ord.timestamp < ?i GROUP BY subs.subscription_id HAVING COUNT(subs.subscription_id) < ?i AND MAX(ord.order_id) IN(SELECT ord2.order_id FROM ?:orders as ord2 WHERE ord2.status = 'F')", $start_time, $end_time, $addon_settings['rb_fail_attempts']);
...
function fn_recurring_billing_get_additional_product_data_before_discounts(&$product, $auth, $get_options)
{
	if (AREA != 'A' && !empty($product['product_id']) && empty($product['extra']['recurring_price_calculated']) && empty($product['extra']['parent']) && empty($product['exclude_from_calculate'])) {
		...
		$plan_ids = db_get_fields("SELECT plan_id FROM ?:recurring_plans WHERE status = 'A' AND product_ids LIKE '%".$product['product_id']."%'");
...
function fn_recurring_billing_pre_add_to_cart(&$product_data, &$cart, $auth, $update = false)
{
	foreach ($product_data as $k => $v) {
		...
		if (!empty($product_data[$k]['extra']['recurring_plan_id']) || /*!empty($product_data[$k]['extra']['parent']) ||*/ (isset($v['recurring_plan_id']) && $v['recurring_plan_id'] == 0)) { //FIXME: Recurring billing should work correctly with the product configurator!
			...
			$plans = db_get_array("SELECT plan_id, allow_free_buy FROM ?:recurring_plans WHERE status = 'A' AND product_ids LIKE '%".empty($v['product_id']) ? $k : $v['product_id']."%'");
		...
		if (isset($apply)) {
			foreach ($order_info['items'] as $product) {
				if (!empty($product['extra']['recurring_plan_id'])) {
					...
					$usergroup_ids = db_get_fields("SELECT usergroup_id FROM ?:usergroups WHERE recurring_plans_ids LIKE '%".$product['extra']['recurring_plan_id']."%' AND status = 'A'");
...
function fn_recurring_billing_get_status_data(&$data, $status, $type, $object_id, $lang_code)
{
	if ($type == STATUSES_ORDER && !empty($object_id)) {
		...
		$subscriptions = db_get_array("SELECT subscription_id FROM ?:recurring_subscriptions WHERE order_ids LIKE '%".$object_id."%'");
...
function fn_recurring_billing_buy_together_restricted_product($product_id, $auth, $is_restricted, $show_notification)
{
	...
	$plan = db_get_row("SELECT plan_id, duration FROM ?:recurring_plans WHERE status = 'A' AND product_ids LIKE '%".$product_id."%'");
...
function fn_recurring_billing_get_products(&$params, &$fields, &$sortings, &$condition, &$join, &$sorting, &$group_by)
{
	if (!empty($params['picker_for']) && $params['picker_for'] == 'gift_certificates') {
		...
		$join .= " LEFT JOIN ?:recurring_plans as rec_plan ON rec_plan.product_ids LIKE products.product_id";
