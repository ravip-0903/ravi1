...
if($mode == 'shipment_initiate_list')
{
	$sql = "select * from ?:status_descriptions where type='O'";
	$row_st = db_get_array($sql);
	$view->assign('statusarray',$row_st);
	
	$view->assign('page_title','Bulk Status Change :: Step 1');
	$found = '';
	$not_found = '';
	...
	{	//print_r($_REQUEST);die();
		if(($_REQUEST['text_area'] != '' && $_FILES["file"]["type"] != '') || ($_REQUEST['text_area'] == '' && $_FILES["file"]["type"] == ''))
		{
			...
			return array(CONTROLLER_STATUS_REDIRECT, "shipment_completed.shipment_initiate_list&from=".$_GET['from'] ."&to=".$_GET['to']);
		}
		if($_REQUEST['text_area'] != '')
		{
			...
			$order_ids = explode(" ", trim(str_replace($string," ",$_REQUEST['text_area'])));
				...
				$_SESSION['shipment_order_list']['orders']['data'] = array('order_id' => $order_ids);

				
				unset($_REQUEST['redirect_url']);
				return array(CONTROLLER_STATUS_REDIRECT, "shipment_completed.shipment_order_list&from=".$_GET['from'] ."&to=".$_GET['to']);
		}else{
...
if ($mode == 'shipment_order_list') {
	$order_ids1 = $_SESSION['shipment_order_list']['orders']['data']['order_id'];
	foreach($order_ids1 as $k=>$v)
			{
				$statusfrom = "'" . implode("','", explode(",",$_GET['from']) ). "'";
				$query = "SELECT * FROM cscart_orders where order_id='".$v."' and status in (".$statusfrom.")";
	...
	if($not_found_imp != '')
	{
		...
		$view->assign('page_title','Bulk Status Change :: Step 2');
	}
}

if ($mode == 'shipment_completed_status') {
	$order_ids = $_SESSION['shipment_order_list']['orders']['data']['order_id'];
	$cnt = count($order_ids);
	if($_GET["C"])
	{
		$notify['C'] = true;	
	...
	foreach($order_ids as $k => $v)
	{
		...
		fn_change_order_status($v, $_REQUEST['to'], '', $notify);
	}
	fn_set_notification('N','Order',$cnt.' orders has change status to ['. $_REQUEST['to'] .'] successfully.','I');
	unset($_REQUEST['redirect_url']);
	return array(CONTROLLER_STATUS_REDIRECT, "shipment_completed.shipment_initiate_list&from=".$_REQUEST['from'] ."&to=".$_REQUEST['to']);
}
