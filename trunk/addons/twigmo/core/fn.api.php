...
//
// $Id: fn.api.php 12865 2011-07-05 06:57:22Z 2tl $
//
...
function fn_get_as_api_object($object_type, $object,  $fields = array())
{
	$schema = fn_get_schema('api', $object_type, 'php', false);
	return fn_convert_to_schema($schema, $object, $fields);
}

function fn_convert_to_schema($schema, $object, $fields)
{
	$api_data = array();
			...
			$api_data[$field_id] = $object[$field_info['db_field']];
			 ...
			 if (function_exists($field_info['process_get']['func'])) {
				...
				if (!empty($field_info['process_get']['params'])) {
					$args = fn_get_schema_params($field_info['process_get']['params'], $object, $api_data);
				}
...
// orders schema funcs
function fn_api_orders_get_data($order_id, $type, $object_type, $data, $single = true)
{
	if (empty($data)) {
