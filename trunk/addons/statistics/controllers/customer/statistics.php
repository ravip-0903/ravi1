...
//
// $Id: statistics.php 11139 2010-11-08 12:18:16Z klerik $
//
...
function fn_stat_save_session_data(&$stat_data)
{
	...
	if (!empty($stat_data['browser'])) {
		$browser_id = db_get_field("SELECT browser_id FROM ?:stat_browsers USE INDEX (brow_vers_idx) WHERE browser = ?s AND version = ?s", $stat_data['browser'], $stat_data['browser_version']);
		if (empty($browser_id)) {
