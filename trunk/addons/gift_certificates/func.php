...
function fn_add_gift_certificate_log_record($gift_cert_id, $before_info, $after_info, $order_id = 0,$staff_notes='',$user_id='')
{
	if($user_id!='')
	{
		$user_id=$user_id;
...
function fn_delete_cart_gift_certificate(&$cart, $gift_cert_id)
{
	if (!empty($gift_cert_id)) {
		...
		if (empty($cart['gift_certificates'])) {
			...
			/*unset company id 0 when gift certificates is empty*/
			unset($cart['companies']['0']);
			/*unset company id 0 when gift certificates is empty*/
		}
	}
}
...
function fn_delete_gift_certificate_in_use($gift_cert_code, &$cart)
{
	if (!empty($gift_cert_code)) {
		...
		if(isset($_SESSION[$gift_cert_code]))
		unset($_SESSION[$gift_cert_code]);
	}
}
...
function fn_gift_certificates_save_cart(&$cart, $user_id, $type = 'C')
{
	...
	if (!empty($cart['gift_certificates']) && is_array($cart['gift_certificates'])) {
		...
		foreach ($_cart_gift_cert as $_item_id => $_gift_cert) {
			...
			$_cart_gift_cert[$_item_id]['user_type'] = $user_type = empty($_SESSION['auth']['user_id']) ? 'U' : 'R';
			...
			if (!empty($_cart_gift_cert[$_item_id])) {
				//db_query('REPLACE INTO ?:user_session_products ?e', $_cart_gift_cert[$_item_id]);
                            ...
                            if(empty($result)){
                                db_query("INSERT INTO ?:user_session_products ?e", $_cart_gift_cert[$_item_id]);
                            }else{
                                db_query("UPDATE ?:user_session_products SET ?u WHERE user_id = ?i AND type = 'C' AND item_id = ?i AND user_type = ?s", $_cart_gift_cert[$_item_id], $_cart_gift_cert[$_item_id]['user_id'],$_cart_gift_cert[$_item_id]['item_id'],$_cart_gift_cert[$_item_id]['user_type']);
                            }
			}
		}
	}
...
function fn_gift_certificates_place_order($order_id, $fake, $fake1, &$cart)
{
	if (!empty($order_id)) {
		...
		if (isset($cart['gift_certificates'])) {
			...
			/*added by chandan to create the gift certificate as separte order for shopclues merchant*/
			if($cart['companies']['0'] == '0'){
				$order_data = array(
					'order_id' => $order_id,
...
//<-- FIXME: optimize this code
			...
			db_query("REPLACE INTO ?:order_data ?e", $order_data);
			fn_put_total_gc_used_in_orders($order_id, $use_gift_certificates);
		}
	}
...
function fn_get_gift_certificate_balance($gc_id)
{
	$sql="select debit from cscart_gift_certificates_log where gift_cert_id='".$gc_id."' order by log_id desc limit 0,1";
	$bal=db_get_field($sql);
	if(!empty($bal))
	{
		return $bal ;
	}
	...
	{
		$sql="select amount from cscart_gift_certificates where gift_cert_id='".$gc_id."'";
		return db_get_field($sql);
	}
}
function fn_get_gift_certificate_status($gc)
{
	$sql="select status from cscart_gift_certificates where gift_cert_code='".$gc."'";
	$sta=db_get_field($sql);
	if($sta!='A')
	return false;
	else
	return true;
...
function fn_get_siblink_order($order_id,$parent_id)
{
	$sql="select order_id from cscart_orders where parent_order_id='".$parent_id."' and order_id!='".$order_id."' and status in ('".implode("','",Registry::get('config.left_for_gc_order'))."')";
	return db_get_array($sql);
}
function fn_get_paid_siblink_order_data($order_id,$parent_id)
{
	$sql="select data from cscart_order_data cod inner join cscart_orders o on o.order_id=cod.order_id where o.parent_order_id='".$parent_id."' and o.order_id!='".$order_id."' and o.status not in ('".implode("','",Registry::get('config.left_for_gc_order'))."') and cod.type='U'";
	return db_get_field($sql);
}
function fn_apply_gift_certificate($order_info,$status_to,$status_from)
{
	$order_id=$order_info['order_id'];
	if(isset($order_info['parent_order_id']) && $order_info['parent_order_id']!=0)
	{
		$siblink_ids=array();
		$siblink_ids=fn_get_siblink_order($order_id,$order_info['parent_order_id']);
                if(empty($siblink_ids))
		{
			$paid_siblink_data=unserialize(fn_get_paid_siblink_order_data($order_id,$order_info['parent_order_id']));
			if(!empty($paid_siblink_data))
			{
				foreach($order_info['use_gift_certificates'] as $k=>$v)
				{
					$cur_bal=fn_get_gift_certificate_balance($v['gift_cert_id']);
					$order_info['use_gift_certificates'][$k]['amount']=$cur_bal;
				}
			}
		}
                foreach ($order_info['use_gift_certificates'] as $k=>$v)
		{
			$vaild=fn_get_gift_certificate_status($k);
			if(!$vaild)
			{
				if(AREA=='A')
				{
					fn_set_notification('E','',fn_get_lang_var('invalid_gc_status'));
					return 0;
				}
				else
				{
					return 2;//this is for indcating that error from customer end in changing status
				}
			}
			else
			{
				...
				if($cur_bal<$v['cost'])
				{
					if(AREA=='A')
					{
						fn_set_notification('E','',fn_get_lang_var('invalid_gc_amount'));
						return 0;
					}
					else
					{
			...
			$log_id = fn_add_gift_certificate_log_record($v['gift_cert_id'], $before_info, $after_info, $order_id,'',$order_info['user_id']);
			...
			db_query("REPLACE INTO ?:order_data ?e", $order_data);
	      		fn_put_total_gc_used_in_orders($order_id, $use_gift_certificates);
			//update gc amount in all siblink
			if(!empty($siblink_ids))
			{
			 
			 foreach($siblink_ids as $orderid)
			 {
				 $order_data="";
				 $use_gc_info=array();
				 
				 $sql="select data from cscart_order_data where type='U' and order_id=".$orderid['order_id'];
				 $use_gc_info=unserialize(db_get_field($sql));
				 $bal=fn_get_gift_certificate_balance($v['gift_cert_id']);
				 $use_gc_info[$k]['amount']=$bal;
				 $order_data = array(
					'order_id' => $orderid['order_id'],
					'type' => 'U',
					'data' => serialize($use_gc_info)
				);
				db_query("REPLACE INTO ?:order_data ?e", $order_data);
				fn_put_total_gc_used_in_orders($orderid['order_id'], $use_gc_info);
			 }
			}
	...
	else
	{
		foreach ($order_info['use_gift_certificates'] as $k=>$v)
		{
			...
			$after_info = array(
								'amount' 	=> fn_format_price($cur_bal - $v['cost']),
								'products'  => serialize(!empty($debit_products[$k]['products']) ? $debit_products[$k]['products'] : array())
								);
			$log_id = fn_add_gift_certificate_log_record($v['gift_cert_id'], $before_info, $after_info, $order_id,'',$order_info['user_id']);
			...
			if (floatval($cur_bal - $v['cost']) <= 0 &&  !isset($debit_products[$k]['products'])) {
					fn_change_gift_certificate_status($v['gift_cert_id'], 'U');
				}
...
function fn_remove_gift_certificate($order_info)
{
	if(isset($order_info['parent_order_id']) && $order_info['parent_order_id']!=0)
	{
		foreach ($order_info['use_gift_certificates'] as $k => $v) {
			...
			$log_id = fn_add_gift_certificate_log_record($v['gift_cert_id'], $before_info, $after_info, $order_info['order_id'],'',$_SESSION['auth']['user_id']);
			
			$use_gc_info=array();
			 $sql="select data from cscart_order_data where type='U' and order_id=".$order_info['order_id'];
			 $use_gc_info=unserialize(db_get_field($sql));
			 $bal=fn_get_gift_certificate_balance($v['gift_cert_id']);
			 $use_gc_info[$k]['amount']=$bal;
			 $order_data = array(
			...
			db_query("REPLACE INTO ?:order_data ?e", $order_data);
			fn_put_total_gc_used_in_orders($order_info['order_id'], $use_gc_info);
			$siblinks=fn_get_siblink_order($order_info['order_id'],$order_info['parent_order_id']);
			foreach($siblinks as $orderid)
			{
				$order_data="";
				 $use_gc_info=array();
				 $sql="select data from cscart_order_data where type='U' and order_id=".$orderid['order_id'];
	...
	else
	{
		foreach ($order_info['use_gift_certificates'] as $k => $v) {
			...
			$use_gift_certificates[$k] = array(
									'gift_cert_id' => $v['gift_cert_id'],
									'amount' 	=> $cur_bal + $v['cost'],
									'cost' => $v['cost'],
									'log_id' => $log_id
								);
		
			db_query("update ?:order_data set data='".serialize($use_gift_certificates)."' where order_id='".$order_info['order_id']."' and type='U'");
			//db_query("REPLACE INTO ?:order_data ?e", $order_data);
			db_query("UPDATE ?:gift_certificates SET status = 'A' WHERE gift_cert_id = ?i", $v['gift_cert_id']);
		}
        
		//db_query("DELETE FROM ?:gift_certificates_log WHERE order_id = ?i", $order_info['order_id']);
	...
	if (!empty($cart['use_gift_certificates'])) {
		...
		foreach ($cart['use_gift_certificates'] as $code => $value) {
			...
			if (is_array($value) && isset($value['log_id'])) {// indicates that the order is being edited
				...
				$last_log_item = db_get_row("SELECT log_id, debit, debit_products FROM ?:gift_certificates_log WHERE gift_cert_id = ?i ORDER BY timestamp DESC,order_id desc", $value['gift_cert_id']);
			...
			// This step is performed only on Create order and in the customer area.
				...
				$debit_balance = db_get_row("SELECT debit AS amount, debit_products as products FROM ?:gift_certificates_log WHERE gift_cert_id = ?i ORDER BY timestamp DESC,order_id desc", $gift_cert_data['gift_cert_id']);
...
function fn_gift_certificates_calculate_cart(&$cart, &$cart_products, &$auth)
{
	...
	if (!empty($cart['gift_certificates'])) {
		/*added by chandan to stop calculation of GC for merchant orders*/
		...
		/*added by chandan to stop calculation of GC for merchant orders*/			
			...
			if(!array_key_exists($cart['companies']['0'])){
				$cart['companies']['0'] = '0';
			}
		/*added by chandan to start calculation of GC for GC orders for shopclues merchant*/
		}else{
			if((empty($cart['products']) && empty($cart['companies'])) || (count($cart['companies'])  == '1' && array_key_exists('0',$cart['companies']))){
				foreach ((array)$cart['gift_certificates'] as $k => $v) {
					$cart['gift_certificates'][$k]['subtotal'] = $v['amount'];
					$cart['gift_certificates'][$k]['display_subtotal'] = $v['amount'];
					$cart['gift_certificates'][$k]['tax_value'] = 0;
		...
		if (!empty($cart['use_gift_certificates'])){
			...
			unset($cart['pending_certificates']);
			return;
		}
		/*added by chandan to start calculation of GC for GC orders for shopclues merchant*/		
	}
	...
	if (!empty($cart['use_gift_certificates'])) {
       ...
       foreach($cart_products as $k=>$v)
	   {
		...
		foreach ((array)$cart['use_gift_certificates'] as $code => $value) {
			...
			if(!isset($_SESSION[$code]['parent_gc_amount']) && count(array_unique($comp))>1 )
			{
				$_SESSION[$code]['parent_gc_amount']=$value['amount'];
			}			
			
			if(isset($_SESSION[$code]['company_part_for_gc']) && isset($_SESSION[$code]['parent_gc_amount']))
			{
				$_subtotal -= floor(($_SESSION[$code]['parent_gc_amount']*$_SESSION[$code]['company_part_for_gc'])/100);
				if ($_subtotal >= 0) {
					$cart['use_gift_certificates'][$code]['cost'] = floor(($_SESSION[$code]['parent_gc_amount']*$_SESSION[$code]['company_part_for_gc'])/100);
				} else {
					$cart['use_gift_certificates'][$code]['cost'] = floor((($_SESSION[$code]['parent_gc_amount']*$_SESSION[$code]['company_part_for_gc'])/100) + $_subtotal);
					$_subtotal = 0;
				}
				$cart['use_gift_certificates'][$code]['amount']=$_SESSION[$code]['parent_gc_amount'];
...
function fn_get_gift_certificates($params, $items_per_page = 0)
{
	...
	$sortings = array (
		...
		'order_ids' => "?:gift_certificates.order_ids",
	...
	if (isset($params['gift_cert_code']) && fn_string_no_empty($params['gift_cert_code'])) {
		$gc_code=explode(',',trim($params['gift_cert_code']));
		$gc_code=implode("','",$gc_code);

		$condition .= db_quote(" AND ?:gift_certificates.gift_cert_code IN ('".$gc_code."')");
	}
	...
	$total_purchase_balance=db_get_field("SELECT SUM(amount) FROM ?:gift_certificates WHERE 1 $condition");
	
	$cond_result=db_get_array("SELECT gift_cert_id,amount FROM ?:gift_certificates WHERE 1 $condition");
	$used_sum=0;
	foreach($cond_result as $v)
	{
		$gc_used=db_get_field("SELECT debit FROM ?:gift_certificates_log WHERE gift_cert_id = ?i ORDER BY log_id desc", $v['gift_cert_id']);
		
		if(empty($gc_used))
		$used_sum+=0;
		else
		$used_sum+=$v['amount']-$gc_used;
	}
	
	$results_sum['purchase']=$total_purchase_balance;
	$results_sum['used']=$used_sum;
	...
	return array($gift_certificates, $params,$results_sum);
}
...
{
	if (($order_info['status'] == 'N' || $order_info['status'] == 'F') && !empty($order_info['use_gift_certificates'])) {
...
function fn_put_total_gc_used_in_orders($order_id, $used_gc){
	$gc_used = 0;
	foreach($used_gc as $gc){
		$gc_used += $gc['cost'];
	}    
	$sql = "update cscart_orders set gc_used=".$gc_used." where order_id=".$order_id;
	db_query($sql);
}
?>
