...
function fn_check_compatibilities_process_response(data){
	select_first = true;
	for (var i in data.aviable) {			
		fn_change_visibillyty(data.aviable[i].group_id, data.aviable[i].product_id, false);			
	}
	
	for (var i in data.disaviable) {
		fn_change_visibillyty(data.disaviable[i].group_id, data.disaviable[i].product_id, true);		
	}	
}    
...
function fn_check_compatibilities(group_id, product_id, type)
{
	...
	$('[id=group_' + group_id + ']:not(.cm-configurator-disabled) option').removeAttr('disabled');
	
	jQuery.ajaxRequest(fn_url(index_script + '?dispatch=products.compability&product_id=' + initial_product_id + '&group_id='+group_id), {
        hidde: true,
        method: 'get',
